...
public class AbyssGaze extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			if (SoIManager.getInstance().getCurrentStage() == 2 || SoIManager.getInstance().getCurrentStage() == 5)
			{
				htmltext = "32540-2.htm";
			}
			else if (SoIManager.getInstance().getCurrentStage() == 3 && SoIManager.getInstance().isSeedOpen())
			{
		...
		{
			if (SoIManager.getInstance().getCurrentStage() == 3)
			{
				SoIManager.getInstance().teleportInSeed(player);
				return null;
			}
		}
		return htmltext;
