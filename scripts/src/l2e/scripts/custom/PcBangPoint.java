	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (POINTSSKILL.containsKey(event))
		{
			...
			else if (player.getPcBangPoints() >= POINTSSKILL.get(event)[2])
			{
				...
				final SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.USING_S1_PCPOINT);
		...
		else if (TELEPORTERS.containsKey(event))
		{
			if (player.getPcBangPoints() >= TELEPORTERS.get(event)[3])
			{
				...
				{
					BotFunctions.getInstance().getAutoTeleportById(player, player.getLocation(), new Location(TELEPORTERS.get(event)[0], TELEPORTERS.get(event)[1], TELEPORTERS.get(event)[2]), 1000);
					return null;
				}
				player.teleToLocation(TELEPORTERS.get(event)[0], TELEPORTERS.get(event)[1], TELEPORTERS.get(event)[2], true, player.getReflection());
				return null;
			}
			htmltext = "nopoint.htm";
		}
