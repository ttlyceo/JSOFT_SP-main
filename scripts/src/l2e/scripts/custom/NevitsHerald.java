...
public class NevitsHerald extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("text_spam"))
		{
			...
			npc.broadcastPacketToOthers(new NpcSay(NevitsHerald, Say2.SHOUT, NevitsHerald, spam[getRandom(0, spam.length - 1)]));
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if (!isActive)
		{
			...
			{
				final Npc herald = addSpawn(NevitsHerald, _spawn[0], _spawn[1], _spawn[2], _spawn[3], false, 0);
				if (herald != null)
				{
					spawns.add(herald);
				}
