...
public class _660_AidingtheFloranVillage extends Quest
{
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		final long EYES = st.getQuestItemsCount(WATCHING_EYES);
		final long SCALE = st.getQuestItemsCount(DELU_LIZARDMAN_SCALE);
		final long SHARD = st.getQuestItemsCount(ROUGHLY_HEWN_ROCK_GOLEM_SHARD);
		...
		else if(event.equalsIgnoreCase("30291-11.htm"))
		{
			...
			{
				final long n = 100 - EYES;
				final long t = 100 - SCALE - EYES;
				if(EYES >= 100)
				{
		...
		else if(event.equalsIgnoreCase("30291-12.htm"))
		{
			...
			{
				final long n = 200 - EYES;
				final long t = 200 - SCALE - EYES;
		...
		else if(event.equalsIgnoreCase("30291-13.htm"))
		{
			...
			{
				final long n = 500 - EYES;
				final long t = 500 - SCALE - EYES;
				if(EYES >= 500)
				{
