...
public class _351_BlackSwan extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30916-03.htm") && npc.getId() == 30916)
		{
			if (st.isCreated())
			{
		...
		else if (event.equalsIgnoreCase("30969-02a.htm") && npc.getId() == 30969)
		{
			final long lizardFangs = st.getQuestItemsCount(LIZARD_FANG);
			if (lizardFangs > 0)
			{
		...
		else if (event.equalsIgnoreCase("30969-03a.htm") && npc.getId() == 30969)
		{
			final long barrels = st.getQuestItemsCount(BARREL_OF_LEAGUE);
			if (barrels > 0)
			{
		...
		else if (event.equalsIgnoreCase("30969-06.htm") && npc.getId() == 30969)
		{
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
			final int random = st.getRandom(20);
