...
public class _422_RepentYourSins extends Quest
{
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("1") && st.getCond() >= 10)
		{
		...
		{
			final int pLevel = findPetLvl(player, PENITENTS_MANACLES);
			...
			else
			{
				...
				else
				{
					...
					final int newPkCount = player.getPkKills() - pkRemove;
	...
	public final String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
			final int condition = st.getInt("cond");
			final int npcId = npc.getId();
			final long skulls = st.getQuestItemsCount(SCAVENGER_WERERAT_SKULL);
			final long tails = st.getQuestItemsCount(TUREK_WARHOUND_TAIL);
			final long heart = st.getQuestItemsCount(TYRANT_KINGPIN_HEART);
			final long sacs = st.getQuestItemsCount(TRISALIM_TARANTULAS_VENOM_SAC);
			switch (npcId)
			{
