...
public class _355_FamilyHonor extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30181-2.htm") && npc.getId() == 30181)
		{
			if (st.isCreated())
			{
				st.set("cond", "1");
		...
		else if (event.equalsIgnoreCase("30181-4b.htm") && npc.getId() == 30181)
		{
			final long count = st.getQuestItemsCount(GALIBREDO_BUST);
			if (count > 0)
			{
		...
		else if (event.equalsIgnoreCase("30929-7.htm") && npc.getId() == 30929)
		{
			if (st.getQuestItemsCount(WORK_OF_BERONA) > 0)
			{
				final int appraising = st.getRandom(100);
				if (appraising <= 20)
				{
		...
		else if (event.equalsIgnoreCase("30181-6.htm") && npc.getId() == 30181)
		{
