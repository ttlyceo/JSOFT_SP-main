...
public class _620_FourGoblets extends Quest
{
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("15"))
		{
			...
			{
				player.teleToLocation(178298, -84574, -7216, true, player.getReflection());
				htmltext = null;
			}
		...
		else if (event.equalsIgnoreCase("16"))
		{
			if (st.getQuestItemsCount(ANTIQUE_BROOCH) >= 1)
			{
				player.teleToLocation(186942, -75602, -2834, true, player.getReflection());
				htmltext = null;
			}
			else if (st.getQuestItemsCount(GRAVE_PASS) >= 1)
			{
		...
		else if (event.equalsIgnoreCase("17"))
		{
			if (st.getQuestItemsCount(ANTIQUE_BROOCH) >= 1)
			{
				player.teleToLocation(169590, -90218, -2914, true, player.getReflection());
			}
			else
			{
				st.takeItems(GRAVE_PASS, 1);
		...
		else
		{
			try
			{
				final int eventNumber = Integer.parseInt(event);
