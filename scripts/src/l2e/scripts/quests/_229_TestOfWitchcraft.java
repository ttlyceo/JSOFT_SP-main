		...
		switch (event)
		{
			...
			case "30630-14.htm" :
			{
				if (hasQuestItems(player, ALEXANDRIAS_BOOK))
				{
					...
					qs.setCond(4, true);
					qs.addSpawn(DREVANUL_PRINCE_ZERUEL);
					htmltext = event;
				}
				break;
			}
			...
			case "30633-02.htm" :
			{
				...
				if (qs.getLong("spawnDelay") < now)
				{
					qs.set("spawnDelay", (now + 30000L));
					qs.addSpawn(DREVANUL_PRINCE_ZERUEL);
				}
				htmltext = event;
				break;
			...
			switch (npc.getId())
			{
				case NAMELESS_REVENANT :
				{
					if (npc.isScriptValue(0) && hasQuestItems(attacker, ALEXANDRIAS_BOOK, LARAS_MEMO) && !hasQuestItems(attacker, AKLANTOTH_3RD_GEM))
					{
						...
						npc.broadcastPacketToOthers(2000, new NpcSay(npc, Say2.NPC_ALL, NpcStringId.I_ABSOLUTELY_CANNOT_GIVE_IT_TO_YOU_IT_IS_MY_PRECIOUS_JEWEL));
					}
					break;
				}
				...
				case DREVANUL_PRINCE_ZERUEL :
				{
					...
					{
						npc.broadcastPacketToOthers(2000, new NpcSay(npc, Say2.NPC_ALL, NpcStringId.ILL_TAKE_YOUR_LIVES_LATER));
						npc.deleteMe();
						qs.setCond(5, true);
					}
					else if (hasQuestItems(attacker, ORIMS_INSTRUCTIONS, BRIMSTONE_2ND, SWORD_OF_BINDING, SOULTRAP_CRYSTAL))
					{
						...
						{
							npc.setScriptValue(1);
							npc.broadcastPacketToOthers(2000, new NpcSay(npc, Say2.NPC_ALL, NpcStringId.THAT_SWORD_IS_REALLY));
						}
					}
					break;
				}
		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				...
				case DREVANUL_PRINCE_ZERUEL :
				{
					...
					{
						if (killer.getActiveWeaponItem().getId() == SWORD_OF_BINDING)
						{
							npc.broadcastPacketToOthers(2000, new NpcSay(npc, Say2.NPC_ALL, NpcStringId.NO_I_HAVENT_COMPLETELY_FINISHED_THE_COMMAND_FOR_DESTRUCTION_AND_SLAUGHTER_YET));
