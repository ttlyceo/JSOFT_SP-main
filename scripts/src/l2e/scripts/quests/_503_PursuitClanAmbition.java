...
/**
 * Updated by LordWinter 10.07.2012 Based on L2J Eternity-World
 */
public class _503_PursuitClanAmbition extends Quest
{
	...
	public _503_PursuitClanAmbition(int id, String name, String descr)
	{
		...
		addKillId(20974);
		for (final int mobId : drop.keySet())
		{
			addKillId(mobId);
		}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("30645-03.htm"))
		{
			...
			final List<Player> members = player.getClan().getOnlineMembers(player.getObjectId());
			
			for (final Player i : members)
			{
				st = newQuestState(i);
				st.setState(State.STARTED);
			}
		...
		else if (event.equalsIgnoreCase("30766-04.htm"))
		{
			...
			{
				st.set("cond", "9");
				Npc spawnedNpc = addSpawn(30766, 160622, 21230, -3710, 0, false, 90000);
				spawnedNpc.broadcastPacketToOthers(2000, new NpcSay(spawnedNpc.getObjectId(), 0, spawnedNpc.getId(), NpcStringId.BLOOD_AND_HONOR));
				spawnedNpc = st.addSpawn(30759, 160665, 21209, -3710, 0, false, 90000);
				spawnedNpc.broadcastPacketToOthers(2000, new NpcSay(spawnedNpc.getObjectId(), 0, spawnedNpc.getId(), NpcStringId.AMBITION_AND_POWER));
				spawnedNpc = st.addSpawn(30758, 160665, 21291, -3710, 0, false, 90000);
				spawnedNpc.broadcastPacketToOthers(2000, new NpcSay(spawnedNpc.getObjectId(), 0, spawnedNpc.getId(), NpcStringId.WAR_AND_DEATH));
			}
		}
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		final boolean isLeader = player.isClanLeader();
		...
		if ((npc.getMaxHp() / 2) > npc.getCurrentHp())
		{
			if (getRandom(100) < 4)
			{
				if (ImpGraveKepperStat == 1)
				{
					for (int i = 0; i < 19; i++)
					{
						final int x = (int) (100 * Math.cos(i * .785));
						final int y = (int) (100 * Math.sin(i * .785));
				...
				{
					final Collection<Player> plrs = World.getInstance().getAroundPlayers(npc);
					if (!plrs.isEmpty())
					{
						final Player playerToTP = (Player) plrs.toArray()[getRandom(plrs.size())];
						playerToTP.teleToLocation(185462, 20342, -3250, true, player.getReflection());
					}
				}
			}
		}
	...
	public final String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (leader_st != null)
		{
			...
			final int cond = leader_st.getInt("cond");
			...
			if ((cond == droplist.cond) && (getRandom(100) < droplist.chance))
			{
				...
				else
				{
					if (npcId == 27181)
					{
						final Npc spawnedNpc = leader_st.addSpawn(30765, 120000);
						npc.broadcastPacketToOthers(2000, new NpcSay(spawnedNpc.getObjectId(), 0, spawnedNpc.getId(), NpcStringId.CURSE_OF_THE_GODS_ON_THE_ONE_THAT_DEFILES_THE_PROPERTY_OF_THE_EMPIRE));
	...
	private void exit(boolean complete, QuestState st)
	{
		...
		try
		{
			...
			for (final Player i : members)
			{
				...
				final QuestState qs = i.getQuestState(qn);
				if (qs != null)
				{
					qs.exitQuest(true);
				}
	...
	{
		final int clan = st.getPlayer().getClan().getId();
	...
	private void suscribeMembers(QuestState st)
	{
		...
		{
			final PreparedStatement insertion = con.prepareStatement("REPLACE INTO character_quests (charId,name,var,value) VALUES (?,?,?,?)");
			offline.setInt(1, clan);
			final ResultSet rs = offline.executeQuery();
			while (rs.next())
			{
	...
	private int getLeaderVar(QuestState st, String var)
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection())
		{
			...
			final PreparedStatement offline = con.prepareStatement("SELECT value FROM character_quests WHERE charId=? AND var=? AND name=?");
	...
	{
		new _503_PursuitClanAmbition(503, qn, "");
	}
}