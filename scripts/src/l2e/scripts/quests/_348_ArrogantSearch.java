...
/**
 * Created by LordWinter 04.08.2011 Based on L2J Eternity-World
 */
public class _348_ArrogantSearch extends Quest
{
	...
	public _348_ArrogantSearch(int id, String name, String descr)
	{
		...
		for (final int i : BLOODY_OWNERS.keySet())
		{
			addTalkId(i);
		}
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		if (npc.getId() == HANELLIN)
		{
			...
			else if (cond == 25)
			{
				...
				final int lowbgrade = getRandom(10) + 4103;
			...
			else if (cond == 27)
			{
				...
				final int highbgrade = getRandom(8) + 4113;
		...
		else if (cond == 5)
		{
			...
			if (ARK_OWNERS.containsKey(npcId))
			{
				if (st.getQuestItemsCount((Integer) ARK_OWNERS.get(npc.getId())[0]) == 1)
				{
					...
					final int[] i = (int[]) ARK_OWNERS.get(npcId)[5];
	...
	public String onAttack(Npc npc, Player player, int damage, boolean isSummon, Skill skill)
	{
		...
		{
			final int cond = (Integer) ATTACK_DROPS_24.get(npcId)[0];
			final int chance = (Integer) ATTACK_DROPS_24.get(npcId)[3];
		...
		{
			final int cond = (Integer) ATTACK_DROPS_25.get(npcId)[0];
			final int chance = (Integer) ATTACK_DROPS_25.get(npcId)[3];
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
			...
			{
				final int cond = (Integer) DROPS.get(npcId)[0];
			...
			{
				final int cond = (Integer) DROPS_29.get(npcId)[0];
			...
			{
				npc.broadcastPacketToOthers(1000, new NpcSay(npc, Say2.NPC_ALL, NpcStringId.HA_THAT_WAS_FUN_IF_YOU_WISH_TO_FIND_THE_KEY_SEARCH_THE_CORPSE));
			}
		}
		return super.onKill(npc, player, isSummon);
	}
