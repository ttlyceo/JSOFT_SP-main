...
public final class _120_PavelsResearch extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if(event.equalsIgnoreCase("32045-02.htm"))
		{
			if (st.isCond(14))
			{
				...
				npc.broadcastPacketToOthers(2000, new MagicSkillUse(npc, player, 5073, 5, 1500, 0));
			}
		}
		...
		{
			if (player.getLevel() >= 70 && st.isCreated())
			{
				st.set("cond", "1");
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		final QuestState qs = player.getQuestState("_114_ResurrectionOfAnOldManager");
