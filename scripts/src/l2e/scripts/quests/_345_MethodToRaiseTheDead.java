...
public class _345_MethodToRaiseTheDead extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30970-03.htm") && npc.getId() == 30970)
		{
			if (st.isCreated())
			{
				st.set("cond", "1");
		...
		else if (event.equalsIgnoreCase("30970-06.htm") && npc.getId() == 30970)
		{
			if (st.isCond(1))
			{
				st.set("cond", "2");
		...
		else if (event.equalsIgnoreCase("30912-04.htm") && npc.getId() == 30912)
		{
		...
		else if (event.equalsIgnoreCase("30973-04.htm") && npc.getId() == 30973)
		{
			if (st.isCond(3))
			{
		...
		else if (event.equalsIgnoreCase("30971-02a.htm") && npc.getId() == 30971)
		{
		...
		else if (event.equalsIgnoreCase("30971-03.htm") && npc.getId() == 30971)
		{
			if (st.getQuestItemsCount(USELESS_BONE_PIECES) > 0)
			{
				final long amount = st.getQuestItemsCount(USELESS_BONE_PIECES) * 104;
			...
			case State.STARTED:
				switch (npc.getId())
				{
					case Dorothy:
						...
						{
							final long amount = st.getQuestItemsCount(USELESS_BONE_PIECES) * 70;
							st.takeItems(USELESS_BONE_PIECES, -1);
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
			...
			{
				final int randomPart = CORPSE_PARTS[getRandom(CORPSE_PARTS.length)];
