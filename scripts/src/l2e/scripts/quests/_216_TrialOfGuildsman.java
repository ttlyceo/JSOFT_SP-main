...
public class _216_TrialOfGuildsman extends Quest
{
	...
	{
	        VALKON, NORMAN, ALTRAN, PINTER, DUNING
	};
	...
	{
	        MANDRAGORA_SPROUT, MANDRAGORA_SAPLING, MANDRAGORA_BLOSSOM, SILENOS, STRAIN, GHOUL, DEAD_SEEKER, MANDRAGORA_SPROUT2, BREKA_ORC, BREKA_ORC_ARCHER, BREKA_ORC_SHAMAN, BREKA_ORC_OVERLORD, BREKA_ORC_WARRIOR, ANT, ANT_CAPTAIN, ANT_OVERSEER, GRANITE_GOLEM
	};
	...
	{
	        RP_JOURNEYMAN_RING, ALLTRANS_INSTRUCTIONS, RP_JOURNEYMAN_RING, VALKONS_RECOMMEND, MANDRAGORA_BERRY, ALLTRANS_RECOMMEND1, DUNINGS_KEY, NORMANS_INSTRUCTIONS, NORMANS_LIST, NORMANS_RECEIPT, ALLTRANS_RECOMMEND2, PINTERS_INSTRUCTIONS, RP_AMBER_BEAD, AMBER_BEAD, DUNINGS_INSTRUCTIONS
	};
	...
	public String onTalk(Npc npc, Player talker)
	{
		...
		{
			if (ArrayUtils.contains(CLASSES, talker.getClassId().getId()))
			{
		...
		else if (npcId == ALTRAN && st.getInt("cond") >= 1 && st.getQuestItemsCount(VALKONS_RECOMMEND) == 1 && st.getQuestItemsCount(MANDRAGORA_BERRY) == 0)
		{
			htmltext = "30283-01.htm";
			st.set("cond", "2");
			st.playSound("ItemSound.quest_middle");
		}
		else if (npcId == ALTRAN && st.getInt("cond") >= 1 && st.getQuestItemsCount(VALKONS_RECOMMEND) == 1 && st.getQuestItemsCount(MANDRAGORA_BERRY) == 1)
		{
		...
		else if (npcId == NORMAN && st.getInt("cond") >= 1 && st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) == 1 && st.getQuestItemsCount(ALLTRANS_RECOMMEND1) == 1)
		{
			htmltext = "30210-01.htm";
		}
		else if (npcId == NORMAN && st.getInt("cond") >= 1 && st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(NORMANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(NORMANS_RECEIPT) > 0)
		{
			htmltext = "30210-05.htm";
		}
		else if (npcId == NORMAN && st.getInt("cond") >= 1 && st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(NORMANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(DUNINGS_INSTRUCTIONS) > 0)
		{
			htmltext = "30210-06.htm";
		}
		else if (npcId == NORMAN && st.getInt("cond") >= 1 && st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(NORMANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(DUNINGS_KEY) >= 30)
		{
