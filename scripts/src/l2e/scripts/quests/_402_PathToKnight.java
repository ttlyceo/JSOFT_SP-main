...
public class _402_PathToKnight extends Quest
{
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		String htmltext = event;
        	...
        	final long squire = st.getQuestItemsCount(MARK_OF_ESQUIRE);
        	final long coin1 = st.getQuestItemsCount(COIN_OF_LORDS1);
        	final long coin2 = st.getQuestItemsCount(COIN_OF_LORDS2);
        	final long coin3 = st.getQuestItemsCount(COIN_OF_LORDS3);
        	final long coin4 = st.getQuestItemsCount(COIN_OF_LORDS4);
        	final long coin5 = st.getQuestItemsCount(COIN_OF_LORDS5);
        	final long coin6 = st.getQuestItemsCount(COIN_OF_LORDS6);
        	final long guards_mark1 = st.getQuestItemsCount(GLUDIO_GUARDS_MARK1);
        	final long guards_mark2 = st.getQuestItemsCount(GLUDIO_GUARDS_MARK2);
        	final long guards_mark3 = st.getQuestItemsCount(GLUDIO_GUARDS_MARK3);
        	final long church_mark1 = st.getQuestItemsCount(EINHASAD_CHURCH_MARK1);
        	final long church_mark2 = st.getQuestItemsCount(EINHASAD_CHURCH_MARK2);
        	final long church_mark3 = st.getQuestItemsCount(EINHASAD_CHURCH_MARK3);
        	...
        	else if (event.equalsIgnoreCase("30417-13.htm"))
        	{
            		...
            		{
                		for (final int item : questItemIds)
                		{
                    			st.takeItems(item, -1);
                		}
	...
	public final String onTalk(Npc npc, Player talker)
	{
		...
		if (st == null)
		{
        	...
        	final long church_mark2 = st.getQuestItemsCount(EINHASAD_CHURCH_MARK2);
        	final long church_mark3 = st.getQuestItemsCount(EINHASAD_CHURCH_MARK3);
        	final long coin_count = coin1 + coin2 + coin3 + coin4 + coin5 + coin6;
    	...
    	public String onKill(Npc npc, Player killer, boolean isSummon)
    	{
        	...
        	{
            		final int npcId = npc.getId();
            		final int item_required = DROPLIST.get(npcId)[0];
