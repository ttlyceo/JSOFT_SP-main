...
public class _299_GatherIngredientsForPie extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30620-1.htm") && npc.getId() == 30620)
		{
			if (st.isCreated())
			{
				st.set("cond", "1");
		...
		else if (event.equalsIgnoreCase("30620-3.htm") && npc.getId() == 30620)
		{
			if (st.isCond(2))
			{
				st.set("cond", "3");
		...
		else if (event.equalsIgnoreCase("30063-1.htm") && npc.getId() == 30063)
		{
			if (st.isCond(3))
			{
				st.set("cond", "4");
		...
		else if (event.equalsIgnoreCase("30620-5.htm") && npc.getId() == 30620)
		{
			if (st.isCond(4))
			{
				st.set("cond", "5");
		...
		else if (event.equalsIgnoreCase("30466-1.htm") && npc.getId() == 30466)
		{
			if (st.isCond(5))
			{
				st.set("cond", "6");
		...
		else if (event.equalsIgnoreCase("30620-7a.htm") && npc.getId() == 30620)
		{
			if (st.getQuestItemsCount(FRUIT_BASKET) >= 1 && st.isCond(6))
			{
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null && st.getRandom(100) < 50)
		{
