...
public class _123_TheLeaderAndTheFollower extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final ClanMember cm_apprentice = player.getClan().getClanMember(player.getApprentice());
			if (cm_apprentice.isOnline())
			{
				final Player apprentice = cm_apprentice.getPlayerInstance();
				if (apprentice != null)
				{
					final QuestState apQuest = apprentice.getQuestState(qn);
					if (apQuest != null)
					{
						final int crystals = apQuest.getInt("cond") == 3 ? 922 : 771;
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		switch (st.getState())
		{
			case State.CREATED:
				...
				{
					final ClanMember cm = player.getClan().getClanMember(player.getApprentice());
					if ((cm != null) && (cm.isOnline()))
					{
						final Player apprentice = cm.getPlayerInstance();
						if (apprentice != null)
						{
							final QuestState apQuest = apprentice.getQuestState(qn);
							if (apQuest != null)
							{
								final int apCond = apQuest.getInt("cond");
								if (apCond == 3)
								{
	...
	public final String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		switch (npc.getId())
		{
			case BRUIN_LIZARDMAN:
				...
				break;
			case PICOT_ARANEID:
				final ClanMember cmSponsor = player.getClan().getClanMember(sponsor);
				if ((cmSponsor != null) && (cmSponsor.isOnline()))
				{
					final Player sponsorHelper = cmSponsor.getPlayerInstance();
