...
public class _622_SpecialtyLiquorDelivery extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			if (st.isCreated() && npc.getId() == 31521)
			{
				st.set("cond", "1");
		...
		{
			if (st.isCond(1) && npc.getId() == 31547)
			{
				st.set("cond", "2");
		...
		{
			if (st.isCond(2) && npc.getId() == 31546)
			{
				st.set("cond", "3");
				st.playSound("ItemSound.quest_middle");
				st.takeItems(DRINK, 1);
		...
		{
			if (st.isCond(3) && npc.getId() == 31545)
			{
				st.set("cond", "4");
				st.playSound("ItemSound.quest_middle");
				st.takeItems(DRINK, 1);
		...
		{
			if (st.isCond(4) && npc.getId() == 31544)
			{
				st.set("cond", "5");
				st.playSound("ItemSound.quest_middle");
				st.takeItems(DRINK, 1);
		...
		{
			if (st.isCond(5) && npc.getId() == 31543)
			{
				st.set("cond", "6");
				st.playSound("ItemSound.quest_middle");
				st.takeItems(DRINK, 1);
		...
		{
			if (st.isCond(6) && npc.getId() == 31521)
			{
				st.set("cond", "7");
				st.playSound("ItemSound.quest_middle");
				st.takeItems(FEE, 5);
		...
		{
			if (st.isCond(7) && npc.getId() == 31267)
			{
				if (getRandom(5) < 1)
				{
