...
public class _905_RefinedDragonBlood extends Quest
{
	...
	public _905_RefinedDragonBlood(int questId, String name, String descr)
	{
		...
		for (final int npc : SEPARATED_SOUL)
		{
			addStartNpc(npc);
			addTalkId(npc);
		}
		
		for (final int first_group : BLUE)
		{
			addKillId(first_group);
		}
		
		for (final int second_group : RED)
		{
		...
		{
		        UNREFINED_BLUE_DRAGON_BLOOD, UNREFINED_RED_DRAGON_BLOOD
		};
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (ArrayUtils.contains(SEPARATED_SOUL, npc.getId()))
		{
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		{
			if (ArrayUtils.contains(BLUE, npc.getId()) && getRandom(100) < DROP_CHANCE && st.getQuestItemsCount(UNREFINED_BLUE_DRAGON_BLOOD) < 10)
			{
				st.giveItems(UNREFINED_BLUE_DRAGON_BLOOD, 1);
				st.playSound("ItemSound.quest_itemget");
			}
			else if (ArrayUtils.contains(RED, npc.getId()) && getRandom(100) < DROP_CHANCE && st.getQuestItemsCount(UNREFINED_RED_DRAGON_BLOOD) < 10)
			{
