...
{
	private final static String qn = "_419_GetAPet";
	...
	private String checkQuestions(QuestState st)
	{
		...
		int question = 1;
		final String quiz = st.get("quiz");
		final int answers = st.getInt("answers");
		if (answers < 10)
		{
			final List<String> temp = Arrays.asList(quiz.split("\\s"));
			final List<String> questions = new ArrayList<>(temp);
			
			final int index = st.getRandom(questions.size());
			question = Integer.parseInt(questions.get(index));
			questions.remove(index);
			
			String questionsF = "";
			final Iterator<String> i = questions.iterator();
			while (i.hasNext())
			{
	...
	public String onTalk(Npc npc, Player talker)
	{
		...
		if (npcId == PET_MANAGER_MARTIN)
		{
			...
			{
				final long proofs = getCountOfProofs(st);
				if (proofs == 0)
				{
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		{
			final int npcId = npc.getId();
			final long collected = getCountOfProofs(st);
			if (collected < REQUIRED_SPIDER_LEGS)
			{
				final List<Integer> npcs = new ArrayList<>();
				int item = 0;
				final int race = killer.getRace().ordinal();
				switch (race)
				{
					case 0 :
						npcs.add(SPIDER_H1);
