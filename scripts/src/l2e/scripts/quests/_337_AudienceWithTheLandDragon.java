...
public class _337_AudienceWithTheLandDragon extends Quest
{
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon)
	{
		...
		for (final int[] npcInfo : DROP_ON_ATTACK)
		{
			if (npcInfo[0] == npcId)
			{
				...
				if (npc.getCurrentHp() < (npc.getMaxHp() * 0.8) && npc.isScriptValue(0))
				{
					if (st.getInt("drop" + npcInfo[2]) != 1)
					{
						break;
					...
					{
						final Npc mob = addSpawn(npcInfo[5], npc.getX() + getRandom(-150, 150), npc.getY() + getRandom(-150, 150), npc.getZ(), npc.getHeading(), true, 60000L, false);
						mob.setRunning();
	...
	{
		new _337_AudienceWithTheLandDragon(337, qn, "");
	}
}
