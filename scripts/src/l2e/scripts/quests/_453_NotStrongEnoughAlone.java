...
public class _453_NotStrongEnoughAlone extends Quest
{
	...
	public _453_NotStrongEnoughAlone(int questId, String name, String descr)
	{
		...
		for (final int i : Monsters1)
		{
			addKillId(i);
		}
		
		for (final int i : Monsters2)
		{
			addKillId(i);
		}
		
		for (final int i : Monsters3)
		{
			addKillId(i);
		}
	}
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		final QuestState prev = player.getQuestState("_10282_ToTheSeedOfAnnihilation");
		...
		switch (st.getState())
		{
			...
			case State.STARTED :
				...
				{
					final boolean i1 = getRandomBoolean();
	...
	private void increaseNpcKill(Player player, Npc npc)
	{
		...
		if (ArrayUtils.contains(Monsters1, npc.getId()) && st.getInt("cond") == 2)
		{
			int val = 0;
		...
		else if (ArrayUtils.contains(Monsters2, npc.getId()) && st.getInt("cond") == 3)
		{
			int val = 0;
		...
		else if (ArrayUtils.contains(Monsters3, npc.getId()) && st.getInt("cond") == 4)
		{
			int val = 0;
