...
public class _213_TrialOfSeeker extends Quest
{
	...
	{
	        DUFNER, TERRY, BRUNON, VIKTOR, MARINA
	};
	...
	{
	        NEER_GHOUL_BERSERKER, OL_MAHUM_CAPTAIN, TUREK_ORC_WARLORD, ANT_CAPTAIN, TURAK_BUGBEAR_WARRIOR, MARSH_STAKATO_DRONE, BREKA_ORC_OVERLORD, ANT_WARRIOR_CAPTAIN, LETO_LIZARDMAN_WARRIOR, MEDUSA
	};
	...
	{
	        DUFNERS_LETTER, TERYS_ORDER1, TERYS_ORDER2, TERYS_LETTER, VIKTORS_LETTER, HAWKEYES_LETTER, MYSTERIOUS_RUNESTONE, OL_MAHUM_RUNESTONE, TUREK_RUNESTONE, ANT_RUNESTONE, TURAK_BUGBEAR_RUNESTONE, TERYS_BOX, VIKTORS_REQUEST, MEDUSAS_SCALES, SILENS_RUNESTONE, ANALYSIS_REQUEST, MARINAS_LETTER, EXPERIMENT_TOOLS, ANALYSIS_RESULT, TERYS_ORDER3, LIST_OF_HOST, ABYSS_RUNESTONE1, ABYSS_RUNESTONE2, ABYSS_RUNESTONE3, ABYSS_RUNESTONE4, TERYS_REPORT
	};
	...
	static
	{
		...
		{
		        TERYS_ORDER1, MYSTERIOUS_RUNESTONE, 10, 1
		});
		DROPLIST.put(OL_MAHUM_CAPTAIN, new int[]
		{
		        TERYS_ORDER2, OL_MAHUM_RUNESTONE, 25, 1
		});
		DROPLIST.put(TUREK_ORC_WARLORD, new int[]
		{
		        TERYS_ORDER2, TUREK_RUNESTONE, 25, 1
		});
		DROPLIST.put(ANT_CAPTAIN, new int[]
		{
		        TERYS_ORDER2, ANT_RUNESTONE, 25, 1
		});
		DROPLIST.put(TURAK_BUGBEAR_WARRIOR, new int[]
		{
		        TERYS_ORDER2, TURAK_BUGBEAR_RUNESTONE, 25, 1
		});
		DROPLIST.put(MARSH_STAKATO_DRONE, new int[]
		{
		        LIST_OF_HOST, ABYSS_RUNESTONE1, 25, 1
		});
		DROPLIST.put(BREKA_ORC_OVERLORD, new int[]
		{
		        LIST_OF_HOST, ABYSS_RUNESTONE2, 25, 1
		});
		DROPLIST.put(ANT_WARRIOR_CAPTAIN, new int[]
		{
		        LIST_OF_HOST, ABYSS_RUNESTONE3, 25, 1
		});
		DROPLIST.put(LETO_LIZARDMAN_WARRIOR, new int[]
		{
		        LIST_OF_HOST, ABYSS_RUNESTONE4, 25, 1
		});
		DROPLIST.put(MEDUSA, new int[]
		{
		        VIKTORS_REQUEST, MEDUSAS_SCALES, 30, 10
		});
	}
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		final int cond = st.getInt("cond");
		final int npcId = npc.getId();
		final int required = DROPLIST.get(npcId)[0];
		final int item = DROPLIST.get(npcId)[1];
		final int chance = DROPLIST.get(npcId)[2];
		final int maxqty = DROPLIST.get(npcId)[3];
