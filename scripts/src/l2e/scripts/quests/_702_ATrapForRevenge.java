...
public class _702_ATrapForRevenge extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final long count = st.getQuestItemsCount(DrakeFlesh);
			if (count > 0)
			{
		...
		else if (event.equalsIgnoreCase("32563-11.htm"))
		{
			...
			{
				final long count = st.getQuestItemsCount(VariantDrakeWingHorns);
		...
		{
			if (st.isCond(2) && st.getQuestItemsCount(DrakeFlesh) >= 100)
			{
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		final QuestState st = player.getQuestState(qn);
		final QuestState prev = player.getQuestState("_10273_GoodDayToFly");
		if (st == null)
		{
			return htmltext;
		}
