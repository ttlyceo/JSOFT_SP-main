...
public class _999_T1Tutorial extends Quest
{
	...
	private static final Map<Object, Object[]> Event = new HashMap<>();
	...
	public _999_T1Tutorial(int questId, String name, String descr)
	{
		...
		for (final int startNpc : NPCS)
		{
			addStartNpc(startNpc);
		}

		for (final int FirstTalkId : NPCS)
		{
			addFirstTalkId(FirstTalkId);
		}
		for (final int TalkId : NPCS)
		{
			addTalkId(TalkId);
		}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("isle"))
		{
			...
			player.teleToLocation(-120050, 44500, 360, true, player.getReflection());
			htmltext = "<html><body>" + npc.getName(null) + ":<br>" + ServerStorage.getInstance().getString(player.getLang(), "999quest.ISLE") + "</body></html>";
		}
		else
		{
					...
					else if ((step == 1) && (st.getQuestItemsCount(talk.item) == 0) && (Ex <= 2))
					{
						if (st.getQuestItemsCount(BLUE_GEM) != 0)
						{
							...
							if (player.getClassId().isMage())
							{
								...
								if (htmltext.equals(""))
								{
									htmltext = "<html><body>" + ServerStorage.getInstance().getString(player.getLang(), "999quest.CAN_NOT_HELP") + "</body></html>";
								}
							}
							else
							{
								...
								if (htmltext.equals(""))
								{
									htmltext = "<html><body>" + ServerStorage.getInstance().getString(player.getLang(), "999quest.CAN_NOT_HELP1") + "</body></html>";
								}
							}
						}
						else
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if ((Ex <= 2) && (st.getQuestItemsCount(BLUE_GEM) < 1))
		{
			...
			{
				((MonsterInstance) npc).dropSingleItem(player, BLUE_GEM, 1);
				st.playSound("ItemSound.quest_tutorial");
			}
		}
		return null;
