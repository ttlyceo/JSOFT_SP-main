...
public class _420_LittleWings extends Quest
{
	...
	{
	        FRY_STN, FRY_STN_DLX, FSN_LIST, FSN_LIST_DLX, TD_BCK_SKN, JUICE, SCALE_1, EX_EGG, SCALE_2, ZW_EGG, SCALE_3, KA_EGG, SCALE_4, SU_EGG, SCALE_5, SH_EGG, FRY_DUST
	};
	...
	{
	        PM_COOPER, SG_CRONOS, GD_BYRON, MC_MARIA, FR_MYMYU, DK_EXARION, DK_ZWOV, DK_KALIBRAN, WM_SUZET, WM_SHAMHAI
	};
	...
	{
	        TD_LORD, LO_LZRD_W, MS_SPIDER, RD_SCVNGR, BO_OVERLD, DD_SEEKER, FLINE, LIELE, VL_TREANT, SATYR, UNICORN, FR_RUNNER, FL_ELDER, LI_ELDER, VT_ELDER, ST_ELDER, UN_ELDER, SPIRIT_TIMINIEL
	};
	...
	public _420_LittleWings(int questId, String name, String descr)
	{
		...
		for (final int npcTalkerId : TALKERS)
		{
			addTalkId(npcTalkerId);
		}

		for (final int npcToKillId : TO_KILL_ID)
		{
	...
	public String onTalk(Npc npc, Player talker)
	{
		...
		final long _coal = st.getQuestItemsCount(1870);
		final long _char = st.getQuestItemsCount(1871);
		final long _gemd = st.getQuestItemsCount(2130);
		final long _gemc = st.getQuestItemsCount(2131);
		final long _snug = st.getQuestItemsCount(1873);
		final long _sofp = st.getQuestItemsCount(1875);
		final long _tdbk = st.getQuestItemsCount(TD_BCK_SKN);
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if (st != null)
		{
			...
			final int whom = st.getInt("dragon");
			final int progress = st.getInt("progress");
			final long skins = st.getQuestItemsCount(TD_BCK_SKN);
			final long fsn = st.getQuestItemsCount(FSN_LIST);
			int eggs = 0, eggDropper = 0, scale = 0;
			
			if (((cond < 5) && ((st.getQuestItemsCount(FSN_LIST) == 1) && (skins < 10))) || ((st.getQuestItemsCount(FSN_LIST_DLX) == 1) && (skins < 20)))
			{
				if (npcId == TD_LORD)
				{
					final long count = fsn == 1 ? 10 : 20;
					st.dropQuestItems(TD_BCK_SKN, 1, 1, count, false, BACK_DROP, true);
				}
			}
			else if ((cond >= 5) && ((progress == 14) || (progress == 15) || (progress == 21) || (progress == 22)))
			{
				if (whom == 1)
				{
					eggs = EX_EGG;
				...
				final long prevItems = st.getQuestItemsCount(eggs);
			...
			else if ((cond < 5) && (st.getQuestItemsCount(FRY_STN_DLX) == 1))
			{
				if (ArrayUtils.contains(TO_KILL_ID, npcId) && (npcId != SPIRIT_TIMINIEL))
				{
					st.takeItems(FRY_STN_DLX, 1);
					st.set("progress", "7");
