...
public class _171_ActsOfEvil extends Quest
{
	...
	public _171_ActsOfEvil(int questId, String name, String descr)
	{
		...
		{
		        RANGERS_REPORT1, RANGERS_REPORT2, RANGERS_REPORT3, RANGERS_REPORT4, OL_MAHUM_HEAD, CARGOBOX, TYRAS_BILL, CERTIFICATE, BLADE_MOLD, WEAPON_TRADE_CONTRACT
		};
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30381-02.htm") && npc.getId() == 30381)
		{
			if (st.isCreated())
			{
				st.set("cond", "1");
		...
		else if (event.equalsIgnoreCase("30207-02.htm") && cond == 1 && npc.getId() == 30207)
		{
			st.set("cond", "2");
		}
		else if (event.equalsIgnoreCase("30381-04.htm") && cond == 4 && npc.getId() == 30381)
		{
			st.set("cond", "5");
		}
		else if (event.equalsIgnoreCase("30381-07.htm") && cond == 6 && npc.getId() == 30381)
		{
			st.set("cond", "7");
			st.takeItems(WEAPON_TRADE_CONTRACT, 1);
			st.playSound("ItemSound.quest_middle");
		}
		else if (event.equalsIgnoreCase("30437-03.htm") && cond == 8 && npc.getId() == 30437)
		{
			st.giveItems(CARGOBOX, 1);
			st.giveItems(CERTIFICATE, 1);
			st.set("cond", "9");
		}
		else if (event.equalsIgnoreCase("30617-04.htm") && cond == 9 && npc.getId() == 30617)
		{
		...
		if (st != null)
		{
			...
			switch (npc.getId())
			{
				...
				case 20066 :
					if (cond == 10)
					{
						final int heads = (int) st.getQuestItemsCount(OL_MAHUM_HEAD);
