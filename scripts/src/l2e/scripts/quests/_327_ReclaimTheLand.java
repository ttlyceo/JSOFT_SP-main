...
public class _327_ReclaimTheLand extends Quest
{
	...
	public String onTalk(Npc npc, Player player)
	{
		...
		switch (st.getState())
		{
			...
			case State.STARTED:
				if (npcId == PIOTUR)
				{
					final long reward = (st.getQuestItemsCount(TUREK_DOGTAG) * 40) + (st.getQuestItemsCount(TUREK_MEDALLION) * 50);
					if (reward == 0)
					{
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
			...
			final int chance = DROPLIST.get(npcId)[1];
			final int item = DROPLIST.get(npcId)[0];
