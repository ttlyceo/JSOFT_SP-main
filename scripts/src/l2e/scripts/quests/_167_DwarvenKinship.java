...
public class _167_DwarvenKinship extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30350-04.htm") && npc.getId() == 30350)
		{
			if (st.isCreated())
			{
				st.set("cond", "1");
		...
		else if (event.equalsIgnoreCase("30255-03.htm") && npc.getId() == 30255)
		{
			if (st.isCond(1))
			{
				st.set("cond", "2");
		...
		else if (event.equalsIgnoreCase("30255-04.htm") && npc.getId() == 30255)
		{
			if (st.isCond(2))
			{
				st.takeItems(CARLON_LETTER, 1);
		...
		else if (event.equalsIgnoreCase("30210-02.htm") && npc.getId() == 30210)
		{
			if (st.isCond(2))
			{
