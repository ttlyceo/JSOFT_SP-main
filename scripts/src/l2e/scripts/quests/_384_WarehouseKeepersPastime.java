...
public class _384_WarehouseKeepersPastime extends Quest
{
	...
	private final static Map<Integer, Integer> Medal_Chances = new ConcurrentHashMap<>();
	protected final static Map<Integer, Bingo> bingos = new ConcurrentHashMap<>();
	...
	public _384_WarehouseKeepersPastime(int id, String name, String descr)
	{
		...
		for (final int mob : Medal_Chances.keySet())
		{
			addKillId(mob);
		}
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		final long medals = st.getQuestItemsCount(Warehouse_Keepers_Medal);
		...
		else if (event.contains("-game") && (_state == State.STARTED))
		{
			...
			final int char_obj_id = player.getObjectId();
			if (bingos.containsKey(char_obj_id))
			{
				bingos.remove(char_obj_id);
			}
			final Bingo bingo = new BingoClass(big_game, st);
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
			final Integer chance = Medal_Chances.get(npc.getId());
			if (chance != null)
			{
				st.dropQuestItems(Warehouse_Keepers_Medal, 1, Integer.MAX_VALUE, chance * 10000, true);
			}
		}
		return super.onKill(npc, player, isSummon);
	}
	...
	public static class BingoClass extends Bingo
	{
		...
		{
			final int r = getRandom(100);
			for (final int[] l : rew)
			{
				if (r < l[0])
				{
					_qs.giveItems(l[1], l[2]);
					return;
				}
			}
		}
