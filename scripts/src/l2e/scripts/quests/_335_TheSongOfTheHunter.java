...
public class _335_TheSongOfTheHunter extends Quest
{
	...
	private static final int[][][] Items_1st_Circle =
	{
			...
			{
			        {
			                Hakas_Head, Jakas_Head, Markas_Head
					},
					{
					        3
					}
	...
	private static final int[][][] Items_2nd_Circle =
	{
			...
			{
			        {
			                Athus_Head, Lankas_Head, Triskas_Head, Moturas_Head, Kalaths_Head
					},
					{
					        5
					}
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		if (st != null)
		{
				...
				if (request != null)
				{
					...
					if (request.spawnlist.containsKey(npcId) && (st.getQuestItemsCount(request.request_item) < request.request_count))
					{
						...
						if (Rnd.chance(spawn_n_chance[1]))
						{
							...
							{
								npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), 0, npc.getId(), NpcStringId.SHOW_ME_THE_PRETTY_SPARKLING_THINGS_THEYRE_ALL_MINE));
							}
						}
					}
				}
			...
			{
				npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), 0, npc.getId(), NpcStringId.WELL_TAKE_THE_PROPERTY_OF_THE_ANCIENT_EMPIRE));
