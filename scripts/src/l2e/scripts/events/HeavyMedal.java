...
public class HeavyMedal extends AbstractWorldEvent
{
	...
	public HeavyMedal(String name, String descr)
	{
		...
		addStartNpc(31228, 31229);
		addTalkId(31228, 31229);
		addFirstTalkId(31228, 31229);

		_template = WorldEventParser.getInstance().getEvent(9);
	...
	public boolean eventStart(long totalTime, boolean force)
	{
		...
		final ServerMessage msg = new ServerMessage("EventHeavyMedal.START", true);
	...
	public boolean eventStop()
	{
		...
		final ServerMessage msg = new ServerMessage("EventHeavyMedal.STOP", true);
		Announcements.getInstance().announceToAll(msg);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("game"))
		{
			...
			final List<WorldEventReward> requestItems = _template.getVariantRequests().get(level);
		...
		else if (event.equalsIgnoreCase("heads") || event.equalsIgnoreCase("tails"))
		{
			...
			else
			{
				...
				else
				{
					if (level > 0)
					{
						final List<WorldEventReward> rewards = _template.getVariantRewards().get(level - 1);
					...
					calcReward(player, _template, level);
