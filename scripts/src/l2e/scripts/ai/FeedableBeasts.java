...
public class FeedableBeasts extends AbstractNpcAI
{
	...
	{
	        21451, 21452, 21453, 21454, 21455, 21456, 21457, 21458, 21459, 21460, 21461, 21462, 21463, 21464, 21465, 21466, 21467, 21468, 21469, 21470, 21471, 21472, 21473, 21474, 21475, 21476, 21477, 21478, 21479, 21480, 21481, 21482, 21483, 21484, 21485, 21486, 21487, 21488, 21489, 21490, 21491, 21492, 21493, 21494, 21495, 21496, 21497, 21498, 21499, 21500, 21501, 21502, 21503, 21504, 21505, 21506, 21507, 21824, 21825, 21826, 21827, 21828, 21829, 16013, 16014, 16015, 16016, 16017, 16018
	};
	...
	{
	        {
	                NpcStringId.WHAT_DID_YOU_JUST_DO_TO_ME, NpcStringId.ARE_YOU_TRYING_TO_TAME_ME_DONT_DO_THAT, NpcStringId.DONT_GIVE_SUCH_A_THING_YOU_CAN_ENDANGER_YOURSELF, NpcStringId.YUCK_WHAT_IS_THIS_IT_TASTES_TERRIBLE, NpcStringId.IM_HUNGRY_GIVE_ME_A_LITTLE_MORE_PLEASE, NpcStringId.WHAT_IS_THIS_IS_THIS_EDIBLE, NpcStringId.DONT_WORRY_ABOUT_ME, NpcStringId.THANK_YOU_THAT_WAS_DELICIOUS, NpcStringId.I_THINK_I_AM_STARTING_TO_LIKE_YOU, NpcStringId.EEEEEK_EEEEEK
			},
			{
			        NpcStringId.DONT_KEEP_TRYING_TO_TAME_ME_I_DONT_WANT_TO_BE_TAMED, NpcStringId.IT_IS_JUST_FOOD_TO_ME_ALTHOUGH_IT_MAY_ALSO_BE_YOUR_HAND, NpcStringId.IF_I_KEEP_EATING_LIKE_THIS_WONT_I_BECOME_FAT_CHOMP_CHOMP, NpcStringId.WHY_DO_YOU_KEEP_FEEDING_ME, NpcStringId.DONT_TRUST_ME_IM_AFRAID_I_MAY_BETRAY_YOU_LATER
			},
			{
			        NpcStringId.GRRRRR, NpcStringId.YOU_BROUGHT_THIS_UPON_YOURSELF, NpcStringId.I_FEEL_STRANGE_I_KEEP_HAVING_THESE_EVIL_THOUGHTS, NpcStringId.ALAS_SO_THIS_IS_HOW_IT_ALL_ENDS, NpcStringId.I_DONT_FEEL_SO_GOOD_OH_MY_MIND_IS_VERY_TROUBLED
			}
	};
	
	private static final NpcStringId[] TAMED_TEXT =
	{
	        NpcStringId.S1_SO_WHAT_DO_YOU_THINK_IT_IS_LIKE_TO_BE_TAMED, NpcStringId.S1_WHENEVER_I_SEE_SPICE_I_THINK_I_WILL_MISS_YOUR_HAND_THAT_USED_TO_FEED_IT_TO_ME, NpcStringId.S1_DONT_GO_TO_THE_VILLAGE_I_DONT_HAVE_THE_STRENGTH_TO_FOLLOW_YOU, NpcStringId.THANK_YOU_FOR_TRUSTING_ME_S1_I_HOPE_I_WILL_BE_HELPFUL_TO_YOU, NpcStringId.S1_WILL_I_BE_ABLE_TO_HELP_YOU, NpcStringId.I_GUESS_ITS_JUST_MY_ANIMAL_MAGNETISM, NpcStringId.TOO_MUCH_SPICY_FOOD_MAKES_ME_SWEAT_LIKE_A_BEAST, NpcStringId.ANIMALS_NEED_LOVE_TOO
	};
	...
	public void spawnNext(Npc npc, int growthLevel, Player player, int food)
	{
		...
		if (ArrayUtils.contains(TAMED_BEASTS, nextNpcId))
		{
			...
			{
				for (final TamedBeastInstance oldTrained : player.getTrainedBeasts())
				{
					oldTrained.deleteMe();
				}
			}
			...
			final TamedBeastInstance nextNpc = new TamedBeastInstance(IdFactory.getInstance().getNextId(), template, player, food - FOODSKILLDIFF, npc.getX(), npc.getY(), npc.getZ());
			nextNpc.setRunning();
			...
			if (getRandom(20) == 0)
			{
				...
				final NpcSay packet = new NpcSay(nextNpc, 0, message);
				if (message.getParamCount() > 0)
				{
					packet.addStringParameter(player.getName(null));
				}
				npc.broadcastPacketToOthers(1000, packet);
			}
		}
		else
		{
	...
	public String onSkillSee(Npc npc, Player caster, Skill skill, GameObject[] targets, boolean isSummon)
	{
		...
		if (_GrowthCapableMobs.containsKey(npcId))
		{
			...
			if (getRandom(20) == 0)
			{
				...
				final NpcSay packet = new NpcSay(npc, 0, message);
				if (message.getParamCount() > 0)
				{
					packet.addStringParameter(caster.getName(null));
				}
				npc.broadcastPacketToOthers(1000, packet);
			}
		...
		else if (ArrayUtils.contains(TAMED_BEASTS, npcId) && (npc instanceof TamedBeastInstance))
		{
			...
			if (skillId == beast.getFoodType())
			{
				...
				beast.broadcastPacketToOthers(1000, packet);
			}
		}
		return super.onSkillSee(npc, caster, skill, targets, isSummon);
	}
