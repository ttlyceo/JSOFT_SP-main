...
public class FreyaHard extends Fighter
{
	...
	protected void thinkAttack()
	{
		...
		if (!actor.isDead() && !actor.isCastingNow() && (_blizzardReuseTimer < System.currentTimeMillis()))
		{
			final ReflectionWorld instance = ReflectionManager.getInstance().getWorld(actor.getReflectionId());
			if (instance != null)
			{
				if (instance.getAllowed() != null)
				{
					if ((_manaBurnUse < 4) && (actor.getCurrentHp() < (actor.getMaxHp() * (0.8 - (0.2 * _manaBurnUse)))))
					{
						_manaBurnUse++;
						actor.doCast(SkillsParser.getInstance().getInfo(BLIZZARD_FORCE, 1));
						final GameServerPacket packet = new ExShowScreenMessage(NpcStringId.MAGIC_POWER_SO_STRONG_THAT_IT_COULD_MAKE_YOU_LOSE_YOUR_MIND_CAN_BE_FELT_FROM_SOMEWHERE, 2, 3000);
						for (final int objectId : instance.getAllowed())
						{
							final Player activeChar = GameObjectsStorage.getPlayer(objectId);
							if (activeChar != null && activeChar.getReflectionId() == actor.getReflectionId())
							{
								activeChar.setCurrentMp(0);
		...
		if (!actor.isCastingNow() && (_reflectReuseTimer < System.currentTimeMillis()))
		{
			...
			actor.doCast(SkillsParser.getInstance().getInfo(REFLECT_MAGIC, 1));
		...
		{
			actor.setTarget(mostHated);
			actor.doCast(SkillsParser.getInstance().getInfo(ICE_STORM, 1));
	...
	private void aggroPlayers(boolean searchTarget)
	{
		...
		final var instance = ReflectionManager.getInstance().getWorld(actor.getReflectionId());
			...
			if (actor.getReflectionId() == 0)
			{
				...
				if (!players.isEmpty())
				{
					for (final var player : players)
					{
						...
						{
							actor.addDamageHate(player, 0, 2);
							if (searchTarget)
							{
