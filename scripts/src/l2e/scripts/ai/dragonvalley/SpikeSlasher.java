...
public class SpikeSlasher extends Fighter
{
	...
	{
		final Attackable actor = getActiveChar();
		if ((_spawnCount == 0) && (actor.getCurrentHpPercents() < 60))
		{
			_spawnCount++;
			spawnMinions();
		}
		else if ((_spawnCount == 1) && (actor.getCurrentHpPercents() < 20))
		{
			_spawnCount++;
			spawnMinions();
		}
	...
	private void spawnMinions()
	{
		...
		{
			final MonsterInstance minion = NpcUtils.spawnSingle(25733, Location.findPointToStay(actor, 250, true));
