...
public class CabaleBuffer extends AbstractNpcAI
{
	...
	{
	                NpcStringId.THE_DAY_OF_JUDGMENT_IS_NEAR, NpcStringId.THE_PROPHECY_OF_DARKNESS_HAS_BEEN_FULFILLED, NpcStringId.AS_FORETOLD_IN_THE_PROPHECY_OF_DARKNESS_THE_ERA_OF_CHAOS_HAS_BEGUN, NpcStringId.THE_PROPHECY_OF_DARKNESS_HAS_COME_TO_PASS
	};

	public static final NpcStringId[] PREACHER_MSG =
	{
	                NpcStringId.THIS_WORLD_WILL_SOON_BE_ANNIHILATED, NpcStringId.ALL_IS_LOST_PREPARE_TO_MEET_THE_GODDESS_OF_DEATH, NpcStringId.ALL_IS_LOST_THE_PROPHECY_OF_DESTRUCTION_HAS_BEEN_FULFILLED, NpcStringId.THE_END_OF_TIME_HAS_COME_THE_PROPHECY_OF_DESTRUCTION_HAS_BEEN_FULFILLED
	};
	...
	{
		ThreadPoolManager.getInstance().schedule(new CabaleAI(npc), 3000);
		ThreadPoolManager.getInstance().schedule(new Talk(npc), 60000);
	...
	protected class CabaleAI implements Runnable
	{
		...
		public void run()
		{
			...
			for (final Player player : World.getInstance().getAroundPlayers(_npc))
			{
				...
				if ((playerCabal == winningCabal) && (playerCabal != SevenSigns.CABAL_NULL) && (_npc.getId() == SevenSigns.ORATOR_NPC_ID))
				{
					if (!player.isMageClass())
					{
						if (handleCast(player, ORATOR_FIGTER))
						{
							...
							{
								broadcastSay(_npc, NpcStringId.S1_I_GIVE_YOU_THE_BLESSING_OF_PROPHECY, player.getName(null), 500);
							}
							else
							{
					...
					else
					{
						if (handleCast(player, ORATOR_MAGE))
						{
							...
							{
								broadcastSay(_npc, NpcStringId.S1_I_BESTOW_UPON_YOU_THE_AUTHORITY_OF_THE_ABYSS, player.getName(null), 500);
							}
							else
							{
				...
				else if ((playerCabal == losingCabal) && (playerCabal != SevenSigns.CABAL_NULL) && (_npc.getId() == SevenSigns.PREACHER_NPC_ID))
				{
					if (!player.isMageClass())
					{
						if (handleCast(player, PREACHER_FIGTER))
						{
							...
							{
								broadcastSay(_npc, NpcStringId.A_CURSE_UPON_YOU, player.getName(null), 500);
							}
							else
							{
					...
					else
					{
						if (handleCast(player, PREACHER_MAGE))
						{
							...
							{
								broadcastSay(_npc, NpcStringId.S1_YOU_MIGHT_AS_WELL_GIVE_UP, player.getName(null), 500);
							}
							else
							{
								broadcastSay(_npc, NpcStringId.S1_YOU_BRING_AN_ILL_WIND, player.getName(null), 1);
							}
							isBuffALoser = true;
							continue;
						}
			...
			ThreadPoolManager.getInstance().schedule(this, 3000);
		}
	...
	{
		final Effect effect = player.getFirstEffect(skillId);
