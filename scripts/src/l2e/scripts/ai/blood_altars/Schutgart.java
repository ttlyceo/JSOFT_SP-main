 ...
 */
public class Schutgart extends BloodAltarsEngine
{
	...
	public Schutgart(String name, String descr)
	{
		super(name, descr);
		
		restoreStatus(getName());
		_bosses = getBossList(getName());
	...
	{
		if (_bosses.contains(npc.getId()))
		{
			if (getStatus() != 2)
			{
				_status = 2;
				updateStatus(getName(), _status);
			}
		}
	...
	public String onKill(Npc npc, Player player, boolean isSummon)
	{
		...
		{
			_progress++;
			updateProgress(getName(), _progress);
			updateBossStatus(getName(), (RaidBossInstance) npc, 1);
			if (getProgress() >= _bosses.size())
			{
				changeStatus(getName(), getChangeTime(), getStatus());
			}
		}
	...
	{
		new Schutgart(Schutgart.class.getSimpleName(), "ai");
	}
}
