...
public class Parade extends AbstractNpcAI
{
	...
	public Parade(String name, String descr)
	{
		...
		ThreadPoolManager.getInstance().scheduleAtFixedRate(new Start(), diff, cycle);
	}
	...
	protected class Start implements Runnable
	{
		...
		{
			load();
			spawnTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Spawn(), 0, 5000);
			deleteTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Delete(), 10000, 1000);
			cleanTask = ThreadPoolManager.getInstance().schedule(new Clean(), 420000);
		}
	}
