...
public class FreyaBattleEasy extends AbstractReflection
{
	...
	private FreyaBattleEasy()
	{
		...
		addFirstTalkId(18851, 18850);
		addTalkId(32762, 32781, 18851);
		addAttackId(29177, 29179, 18854, 18855);
		addKillId(29177, 25699, 29179, 18855);
		addSpawnId(25699, 29179, 18855);
				...
				switch (event)
				{
					case "openDoor" :
					{
						if (npc.isScriptValue(0))
						{
							...
							{
								if ((players != null) && !players.isDead() && (players.getReflectionId() == world.getReflectionId()))
								{
									final QuestState qs = players.getQuestState("_10286_ReunionWithSirra");
					...
					case "STAGE_2_GLAKIAS" :
					{
						...
						addSpawn(25699, new Location(114707, -114799, -11199, 15956), false, 0, true, world.getReflection());
						break;
					}
					case "STAGE_3_MOVIE" :
					{
					...
					case "STAGE_3_START" :
					{
						...
						world.freya = (GrandBossInstance) addSpawn(29179, new Location(114720, -117085, -11088, 15956), false, 0, true, world.getReflection());
						world.setStatus(4);
						notifyEvent("START_SPAWN", world.controller, null);
						if (!world.freya.isInCombat())
						{
					...
					case "SPAWN_KNIGHT" :
					{
						if (world.canSpawnMobs)
						{
							...
							final Attackable knight = (Attackable) addSpawn(18855, npc.getLocation(), false, 0, false, world.getReflection());
	...
	{
		if (enterInstance(player, npc, new IQCNBWorld(), 139))
		{
		...
		public void run()
		{
			...
			if (inst != null)
			{
				...
				if (canSpawn)
				{
					...
					{
						final Attackable knight = (Attackable) addSpawn(18855, SpawnTerritory.getRandomLoc(CENTRAL_ROOM, false), false, 0, false, _world.getReflection());
