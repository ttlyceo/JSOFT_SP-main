...
public final class FortressOfTheDeadSiege extends ClanHallSiegeEngine
{
	...
	public FortressOfTheDeadSiege(int questId, String name, String descr, final int hallId)
	{
	...
	public Clan getWinner()
	{
		int counter = 0;
		int damagest = 0;
		for (final Entry<Integer, Integer> e : _damageToLidia.entrySet())
		{
	...
	public void startSiege()
	{
		...
		if ((hoursLeft < 0) || (hoursLeft > 6))
		{
			...
			_siegeTask = ThreadPoolManager.getInstance().schedule(new SiegeStarts(), scheduleTime);
		}
		else
		{
			super.startSiege();
	...
	{
		new FortressOfTheDeadSiege(-1, qn, "clanhallsiege", FORTRESS_OF_DEAD);
	}
}
