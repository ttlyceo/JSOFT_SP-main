...
public final class WildBeastReserveSiege extends ClanHallSiegeEngine
{
	...
	public String onFirstTalk(Npc npc, Player player)
	{
		...
		if (npc.getId() == MESSENGER)
		{
			if (!checkIsAttacker(player.getClan()))
			{
				...
				html.setFile(player, player.getLang(), "data/html/scripts/clanhallsiege/" + getName() + "/35627-00.htm");
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.startsWith("Attackers"))
		{
			...
			htmltext = HtmCache.getInstance().getHtm(player, player.getLang(), "data/html/scripts/clanhallsiege/" + getName() + "/35627-14.htm");
			int i = 1;
	...
	private void sendRegistrationPageDate(Player player)
	{
		...
		msg.setFile(player, player.getLang(), "data/html/scripts/clanhallsiege/" + getName() + "/35627-15.htm");
	...
	{
		new WildBeastReserveSiege(-1, qn, "clanhallsiege", BEAST_FARM);
	}
}
