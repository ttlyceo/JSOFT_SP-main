...
public class DwarvenOccupationChange extends Quest
{
	...
	public DwarvenOccupationChange(int questId, String name, String descr)
	{
		...
		for (final int npcId : BH_NPCS)
		{
			addStartNpc(npcId);
			addTalkId(npcId);
		}
		
		for (final int npcId : WS_NPCS)
		{
			addStartNpc(npcId);
			addTalkId(npcId);
		}
		
		for (final int npcId : SCAV_NPCS)
		{
			addStartNpc(npcId);
			addTalkId(npcId);
		}
		
		for (final int npcId : ARTI_NPCS)
		{
			addStartNpc(npcId);
			addTalkId(npcId);
		}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (CLASSES1.keySet().contains(event) || CLASSES2.keySet().contains(event))
		{
			...
			{
				int marks = 0;
				for (final int item : required_marks)
				{
					if (item == 0)
					{
						continue;
				...
				else
				{
					...
					else
					{
						...
						final var rewards = ClassMasterParser.getInstance().getGrandMasterRewards(reward == 8869 ? 1 : 2);
