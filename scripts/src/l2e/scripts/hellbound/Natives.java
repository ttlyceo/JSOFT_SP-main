		...
		if (npc.getId() == TRAITOR)
		{
			if (event.equalsIgnoreCase("open_door"))
			{
				...
				if (qs.getQuestItemsCount(MARK_OF_BETRAYAL) >= 10)
				{
					...
					npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getId(), NpcStringId.ALRIGHT_NOW_LEODAS_IS_YOURS));
					HellboundManager.getInstance().updateTrust(-50, true);

					for (final int doorId : doors)
					{
						final DoorInstance door = DoorParser.getInstance().getDoor(doorId);
						if (door != null)
						{
							door.openMe();
						}
					...
					startQuestTimer("close_doors", 1800000, npc, player);
				}
