...
public class TowerOfNaia extends Quest
{
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("despawn_total"))
		{
			...
			{
				for (final Npc spore : _sporeSpawn)
				{
		...
		else if (event.equalsIgnoreCase("teleport") && (_lock != null))
		{
			...
			if (party == null || party.getMemberCount() < 2 || party.getMemberCount() < 2)
			{
			...
			for (final Player partyMember : party.getMembers())
			{
				...
				{
					partyMember.teleToLocation(-47271, 246098, -9120, true, partyMember.getReflection());
					player.sendMessage("The Tower of Naia countdown has begun. You have only 5 minutes to pass each room.");
				}
			}
			_lock.broadcastPacketToOthers(2000, new MagicSkillUse(_lock, _lock, 5527, 1, 0, 0));
			_lock.deleteMe();
			_lock = null;
			cancelQuestTimers("spawn_lock");
		...
		else if (event.equalsIgnoreCase("go") && _activeRooms.containsKey(npcId) && !_activeRooms.get(npcId))
		{
			...
			if (party != null)
			{
				...
				startRoom(npcId);
				ThreadPoolManager.getInstance().schedule(new StopRoomTask(npcId), 300000);
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon, Skill skill)
	{
		if ((_lock != null) && (npc.getObjectId() == _lock.getObjectId()))
		{
			...
			if ((remaindedHpPercent <= _counter) && (_controller != null))
			{
				if (_counter == 50)
				{
					_lock.getMinionList().addMinion(new MinionData(new MinionTemplate(18493, 1)), true);
				}
				else if (_counter == 10)
				{
					_lock.getMinionList().addMinion(new MinionData(new MinionTemplate(18493, 2)), true);
				}
				_controller.broadcastPacketToOthers(2000, new NpcSay(_controller.getObjectId(), Say2.NPC_ALL, _controller.getId(), NpcStringId.EMERGENCY_EMERGENCY_THE_OUTER_WALL_IS_WEAKENING_RAPIDLY));
				_counter -= 10;
			}
		}
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		else if (Arrays.binarySearch(TOWER_MONSTERS, npcId) >= 0)
		{
			...
			for (final ZoneType zone : ZoneManager.getInstance().getZones(npc.getX(), npc.getY(), npc.getZ()))
			{
		...
		else if (npcId == MUTATED_ELPY)
		{
			...
			((EffectZone) ZoneManager.getInstance().getZoneById(200100)).setIsEnabled(true);
					...
					if ((Math.abs(_indexCount[sporeGroup]) < ELEMENT_INDEX_LIMIT) && (Math.abs(_indexCount[sporeGroup]) > 0) && ((_indexCount[sporeGroup] % 20) == 0) && (getRandom(100) < 50))
					{
						...
						for (final Npc spore : _sporeSpawn)
						{
							if ((spore != null) && !spore.isDead() && (spore.getId() == npcId))
							{
								...
								spore.broadcastPacketToOthers(2000, ns);
							}
						}
					}
	...
	public final String onSpawn(Npc npc)
	{
		...
		if ((npcId == MUTATED_ELPY) && !npc.isTeleporting())
		{
			...
			((EffectZone) ZoneManager.getInstance().getZoneById(200100)).setIsEnabled(false);
			((EffectZone) ZoneManager.getInstance().getZoneById(200101)).setIsEnabled(true);
			((EffectZone) ZoneManager.getInstance().getZoneById(200101)).setIsEnabled(false);
		}
		else if (((npcId == SPORE_BASIC) || ((npcId >= SPORE_FIRE) && (npcId <= SPORE_EARTH))) && (_challengeState == STATE_SPORE_CHALLENGE_IN_PROGRESS))
		{
			...
			npc.getAI().setIntention(CtrlIntention.MOVING, new Location(coord[0], coord[1], coord[2], 0), 0);
	...
	protected void initRoom(int managerId)
	{
		...
		{
			for (final Npc npc : _spawns.get(managerId))
			{
	...
	private void markElpyRespawn()
	{
		...
		ServerVariables.set("elpy_respawn_time", Long.toString(respawnTime));
		ThreadPoolManager.getInstance().schedule(new Runnable()
		{
			@Override
			public void run()
			{
				addSpawn(MUTATED_ELPY, -45480, 246824, -14209, 49152, false, 0, false);
			}
	...
	private int moveTo(Npc npc, int[] coords)
	{
		...
		if (npc != null)
		{
			...
			npc.getAI().setIntention(CtrlIntention.MOVING, new Location(coords[0], coords[1], coords[2], heading), 0);
	...
	{
		final long respawnTime = ServerVariables.getLong("elpy_respawn_time", 0);
	...
	private void removeForeigners(int managerId, Party party)
	{
		if ((party != null) && ZONES.containsKey(managerId) && (ZoneManager.getInstance().getZoneById(ZONES.get(managerId)) != null))
		{
			...
			for (final Player player : zone.getPlayersInside())
			{
				if (player != null)
				{
					final Party charParty = player.getParty();
					if ((charParty == null) || charParty.getMemberCount() < 2 || (charParty.getLeaderObjectId() != party.getLeaderObjectId()))
					{
						player.teleToLocation(16110, 243841, 11616, true, player.getReflection());
					}
				}
			}
		}
	...
	private void removeAllPlayers(int managerId)
	{
		...
		{
			final ZoneType zone = ZoneManager.getInstance().getZoneById(ZONES.get(managerId));
			for (final Player kicked : zone.getPlayersInside())
			{
				if (kicked != null)
				{
					kicked.teleToLocation(17656, 244328, 11595, true, kicked.getReflection());
