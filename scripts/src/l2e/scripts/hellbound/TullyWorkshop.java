...
public class TullyWorkshop extends Quest
{
	...
	public final String onFirstTalk(Npc npc, Player player)
	{
		...
		else if (npcId == DWARVEN_GHOST)
		{
			...
			{
				npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getId(), NpcStringId.HA_HA_YOU_WERE_SO_AFRAID_OF_DEATH_LET_ME_SEE_IF_YOU_FIND_ME_IN_TIME_MAYBE_YOU_CAN_FIND_A_WAY));
				npc.deleteMe();
				return null;
			}
		...
		else if (npcId == AGENT)
		{
			...
			if ((party == null) || party.getMemberCount() < 2 || (party.getLeaderObjectId() != player.getObjectId()))
			{
	...
	public String onTalk(Npc npc, Player player)
	{
		if (npc.getId() == TOMBSTONE)
		{
			...
			for (final Player pl : party.getMembers())
			{
				...
				{
					pl.teleToLocation(26612, 248567, -2856, true, pl.getReflection());
				}
			}
		}
		return null;
	...
	public final String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			final DamageZone dmgZone = (DamageZone) ZoneManager.getInstance().getZoneById(200011);
			if (dmgZone != null)
			{
				dmgZone.setIsEnabled(false);
			}
		}
		...
		{
			npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.DE_ACTIVATE_THE_ALARM));
		...
		{
			for (final Player pl : World.getInstance().getAroundPlayers(npc, 300, 200))
			{
				if (pl != null)
				{
					pl.teleToLocation(-12176, 279696, -10492, true, pl.getReflection());
				}
			}
		...
		{
			for (final Player pl : World.getInstance().getAroundPlayers(npc, 500, 200))
			{
				if (pl != null)
				{
					pl.teleToLocation(-12176, 279696, -10492, true, pl.getReflection());
		...
		if (event.equalsIgnoreCase("enter") && (npcId == DORIAN))
		{
			...
			if ((party != null) && (party.getLeaderObjectId() == player.getObjectId()))
			{
				...
				for (final Player partyMember : party.getMembers())
				{
					partyMember.teleToLocation(-13400, 272827, -15300, true, partyMember.getReflection());
				}
				htmltext = null;
		...
		else if ((event.equalsIgnoreCase("up") || event.equalsIgnoreCase("down")) && TELE_COORDS.containsKey(npcId))
		{
			...
			else
			{
				...
				for (final Player partyMember : party.getMembers())
				{
					...
					{
						partyMember.teleToLocation(tele[0], tele[1], tele[2], true, partyMember.getReflection());
					}
				}
			}
			htmltext = null;
		...
		else if (npcId == AGENT)
		{
			if (event.equalsIgnoreCase("tele_to_7th_floor") && (allowAgentSpawn == false))
			{
				...
				if (party == null || party.getMemberCount() < 2)
				{
					player.teleToLocation(-12501, 281397, -11936, true, player.getReflection());
				...
				else
				{
					...
					else
					{
						for (final Player partyMember : party.getMembers())
						{
							...
							{
								partyMember.teleToLocation(-12501, 281397, -11936, true, partyMember.getReflection());
							}
						}
			...
			else if (event.equalsIgnoreCase("refuse") && (allowAgentSpawn_7th == false))
			{
				...
				for (final MonsterInstance monster : spawnedFollowers)
				{
					if ((monster != null) && !monster.isDead())
					{
						...
						final Player target = player.getParty() == null ? player : player.getParty().getMembers().get(getRandom(player.getParty().getMembers().size()));
		...
		else if (event.equalsIgnoreCase("teleport") && (npcId == DWARVEN_GHOST))
		{
			...
			if (party == null || party.getMemberCount() < 2)
			{
				player.teleToLocation(-12176, 279696, -13596, true, player.getReflection());
			}
			else
			{
				if (party.getLeaderObjectId() != player.getObjectId())
				...
				for (final Player partyMember : party.getMembers())
				{
					if (Util.checkIfInRange(6000, partyMember, npc, true))
					{
						partyMember.teleToLocation(-12176, 279696, -13596, true, partyMember.getReflection());
					}
				}
			}
		}
		else if ((npcId == CUBE_68) && event.startsWith("cube68_tp"))
		{
			...
			if (party != null)
			{
				...
				else
				{
					for (final Player partyMember : party.getMembers())
					{
						if (Util.checkIfInRange(6000, partyMember, npc, true))
						{
							partyMember.teleToLocation(CUBE_68_TELEPORTS[tpId][0], CUBE_68_TELEPORTS[tpId][1], CUBE_68_TELEPORTS[tpId][2], true, partyMember.getReflection());
						}
					}
				}
			}
			...
			{
				player.teleToLocation(CUBE_68_TELEPORTS[tpId][0], CUBE_68_TELEPORTS[tpId][1], CUBE_68_TELEPORTS[tpId][2], true, player.getReflection());
			}
		}
		return htmltext;
	}
	...
	public String onAttack(Npc npc, Player attacker, int damage, boolean isSummon, Skill skill)
	{
		...
		if (Arrays.binarySearch(TELEPORTING_MONSTERS, npcId) >= 0)
		{
			if (Math.abs(npc.getZ() - attacker.getZ()) > 150)
			{
				...
				attacker.teleToLocation(npc.getX() + 50, npc.getY() - 50, npc.getZ(), true, attacker.getReflection());
			}
		}
		else if (((npcId == TEMENIR) || (npcId == KIRETCENAH)) && spawnedFollowers.contains(npc))
		{
			final MonsterInstance victim1 = spawnedFollowers.get(1);
			final MonsterInstance victim2 = spawnedFollowers.get(0);
			final MonsterInstance actor = spawnedFollowers.get(2);
	...
	public String onKill(Npc npc, Player killer, boolean isSummon)
	{
		...
		if (npcId == TULLY)
		{
			...
			final NpcSay ns = new NpcSay(postMortemSpawn.get(0).getObjectId(), Say2.NPC_SHOUT, postMortemSpawn.get(0).getId(), NpcStringId.DETONATOR_INITIALIZATION_TIME_S1_MINUTES_FROM_NOW);
		...
		else if ((npcId == TIMETWISTER_GOLEM) && (_countdown != null))
		{
			...
			{
				npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getId(), NpcStringId.A_FATAL_ERROR_HAS_OCCURRED));
			...
			{
				npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getId(), NpcStringId.TIME_RIFT_DEVICE_ACTIVATION_SUCCESSFUL));
		...
		else if ((npcId >= SERVANT_FIRST) && (npcId <= SERVANT_LAST))
		{
			...
			{
				npc.broadcastPacketToOthers(2000, new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.I_FAILED_PLEASE_FORGIVE_ME_DARION));
			}
			else
			{
				final NpcSay ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.S1_ILL_BE_BACK_DONT_GET_COMFORTABLE);
				ns.addStringParameter(killer.getName(null));
		...
		{
			if (pillarSpawn.getLastSpawn() != null)
			{
	...
	public final String onSpawn(Npc npc)
	{
		if (npc.getId() == TULLY)
		{
			for (final Npc spawnedNpc : postMortemSpawn)
			{
				if (spawnedNpc != null)
				{
					spawnedNpc.deleteMe();
	...
	public String onSpellFinished(Npc npc, Player player, Skill skill)
	{
		...
		{
			player.teleToLocation(21935, 243923, 11088, true, player.getReflection());
		}
	...
	private void do7thFloorDespawn()
	{
		...
		for (final MonsterInstance monster : spawnedFollowerMinions)
		{
			if ((monster != null) && !monster.isDead())
			{
				monster.deleteMe();
	...
	private void handleDoorsOnDeath()
	{
		...
		ThreadPoolManager.getInstance().schedule(new DoorTask(new int[]
		{
		        20250006, 20250007
		}, STATE_OPEN), 2000);
		ThreadPoolManager.getInstance().schedule(new DoorTask(new int[]
		...
		public void run()
		{
			...
			if (countdownTime > 60000)
			{
				if ((countdownTime % 60000) == 0)
				{
					...
					{
						final NpcSay ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getId(), NpcStringId.S1_MINUTES_REMAINING);
						ns.addStringParameter(Integer.toString((countdownTime / 60000)));
						npc.broadcastPacketToOthers(2000, ns);
					}
				}
