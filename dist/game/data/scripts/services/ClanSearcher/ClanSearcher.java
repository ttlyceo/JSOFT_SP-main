...
public class ClanSearcher extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			html.setFile(player, player.getLang(), "data/html/scripts/services/ClanSearcher/newclan.htm");
		...
		{
			html.setFile(player, player.getLang(), "data/html/scripts/services/ClanSearcher/deleteclan.htm");
			player.sendPacket(html);
		}
		return "";
	}
	...
	private void talk(Npc npc, Player player)
	{
		...
		if (npc.getId() == NpcId)
		{
			...
			html.setFile(player, player.getLang(), "data/html/scripts/services/ClanSearcher/1.htm");
	...
	private void sendClanJoinRequest(String event, Npc npc, Player requester)
	{
		...
		final String text = "Player " + requester.getName(null) + " requested to join your clan.";
		...
		for (final ClanMember member : clan.getMembers())
		{
			...
			{
				player.sendPacket(new CreatureSay(player.getObjectId(), 2, player.getName(null), text));
				success = true;
			}
		}
		if (success)
	...
	private void loadSpawnList()
	{
					...
					for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						if (d.getNodeName().equalsIgnoreCase("loc"))
						{
							try
							{
								...
								{
									_log.warn(getScriptName() + " script: " + NpcId + " is wrong NPC id, NPC was not added in spawnlist");
									continue;
								}
		...
		{
			_log.warn(getScriptName() + " script: error reading " + configFile.getAbsolutePath() + " ! " + e.getMessage(), e);
		}
	}
