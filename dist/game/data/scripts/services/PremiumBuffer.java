...
public class PremiumBuffer extends Quest
{
	...
	private String buildHtml(Player player, String buffType)
	{
		    ...
		    Connection con = DatabaseFactory.getInstance().getConnection())
		{
			...
			while (rs.next())
			{
				...
				String bName = SkillsParser.getInstance().getInfo(bId, bLevel).getName(player.getLang());
				bName = bName.replace(" ", "+");
	...
	private String viewAllSchemeBuffs(Player player, String scheme, String page, String action)
	{
		    ...
		    Connection con = DatabaseFactory.getInstance().getConnection())
		{
			if (action.equals("add"))
			{
				...
				while (rss.next())
				{
					String name = SkillsParser.getInstance().getInfo(rss.getInt("buffId"), rss.getInt("buffLevel")).getName(player.getLang());
					name = name.replace(" ", "+");
			...
			else if (action.equals("remove"))
			{
				...
				while (rss.next())
				{
					String name = SkillsParser.getInstance().getInfo(rss.getInt("skill_id"), rss.getInt("skill_level")).getName(player.getLang());
					name = name.replace(" ", "+");
	...
	private String viewAllBuffs(Player player, String type, String typeName, String page)
	{
		final List<String> buffList = new ArrayList<>();
		String HTML_MESSAGE = "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>";
	...
	private String manageSelectedSet(Player player, String id, String newVal, String opt3)
	{
		...
		return viewAllBuffs(player, "set", "Buff Sets", opt3);
	}
		...
		switch (eventParam0)
		{
			...
			case "redirect" :
				switch (eventParam1)
				{
					...
					case "view_buffs" :
						return buildHtml(player, "buff");
					case "view_kamael" :
						return buildHtml(player, "kamael");
					case "view_resists" :
						return buildHtml(player, "resist");
					case "view_songs" :
						return buildHtml(player, "song");
					case "view_dances" :
						return buildHtml(player, "dance");
					case "view_chants" :
						return buildHtml(player, "chant");
					case "view_others" :
						return buildHtml(player, "others");
					case "view_special" :
						return buildHtml(player, "special");
					case "view_cubic" :
						return buildHtml(player, "cubic");
					default :
						if (DEBUG)
						{
							throw new RuntimeException();
			...
			case "manage_scheme_1" :
				return viewAllSchemeBuffs(player, eventParam1, eventParam2, "add");
			case "manage_scheme_2" :
				return viewAllSchemeBuffs(player, eventParam1, eventParam2, "remove");
			...
			case "edit_premium_buff_list" :
				return viewAllBuffs(player, eventParam1, eventParam2, eventParam3);
			...
			case "changeBuffSet" :
				...
				return manageSelectedSet(player, eventParam1, eventParam2, eventParam3);
			case "editSelectedBuff" :
			{
				...
				return viewAllBuffs(player, eventParam3, typeName, page);
			}
			...
			case "cast" :
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (buffs.size() == 0)
					{
						return viewAllSchemeBuffs(player, eventParam1, "1", "add");
					}
					if (!FREE_BUFFS)
					{
			...
			case "giveBuffs" :
			{
				...
				return SMART_WINDOW ? null : buildHtml(player, eventParam3);
			}
			case "castBuffSet" :
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
