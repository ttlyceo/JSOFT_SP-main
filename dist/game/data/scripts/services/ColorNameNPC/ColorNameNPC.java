...
public class ColorNameNPC extends Quest
{
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			htmltext = viewColor(player, Integer.valueOf(value1), player.getName(null));
		}
		...
		else
		{
			...
			for (final String _day : _days)
			{
				...
				for (final Integer _itemId : COLORNAME_NPC_SETTINGS.getFeeItems(days).keySet())
				{
					...
					htmltext += "<font color=\"LEVEL\">" + _count + "</font> " + ItemsParser.getInstance().getTemplate(_itemId).getName(lang) + "<br1>";
				}
				htmltext += "<br><br></td>";
					...
					for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
					{
						if (d.getNodeName().equalsIgnoreCase("loc"))
						{
							try
							{
								...
								{
									_log.warn(getScriptName() + " script: " + NPC + " is wrong NPC id, NPC was not added in spawnlist");
									continue;
								}
