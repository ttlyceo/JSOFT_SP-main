...
public final class StaticObjectsParser extends DocumentParser
{
	...
	public void load()
	{
		...
		parseDatapackFile("data/stats/regions/staticObjects.xml");
		info("Loaded " + _StaticObjectsParser.size() + " staticObject templates.");
	}
	...
	{
		final StaticObjectInstance obj = new StaticObjectInstance(IdFactory.getInstance().getNextId(), new CharTemplate(new StatsSet()), set.getInteger("id"));
		obj.setType(set.getInteger("type", 0));
		obj.setGlobalName(set.getString("name"));
		obj.setMap(set.getString("texture", "none"), set.getInteger("map_x", 0), set.getInteger("map_y", 0));
		final Location loc = new Location(set.getInteger("x"), set.getInteger("y"), set.getInteger("z"));
		loc.correctGeoZ();
