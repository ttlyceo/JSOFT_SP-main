...
public final class PetsParser extends DocumentParser
{
	...
	{
		_pets.clear();
		parseDirectory("data/stats/npcs/pets/", false);
		info("Loaded " + _pets.size() + " pets.");

//		if (true) {
//			try (Connection con = DatabaseFactory.getInstance().getConnection();
//				 PreparedStatement preparedStatement = con.prepareStatement("SELECT * FROM pets_stats");
...
//					PetData petData = _pets.get(typeId);
//					if (petData != null) {
//						PetLevelTemplate petLevelTemplate = petData.getPetLevelData(level);
//						if(petLevelTemplate != null) {
//							if (!DatabaseUtils.set("UPDATE `pets_stats` SET `walk` = ?, `run` = ?, `slowSwim` = ?, `fastSwim` = ?, `slowFly` = ?, `fastFly` = ? WHERE `typeID` = ? AND `level` = ?", petLevelTemplate.getSpeedOnRide(MoveType.WALK), petLevelTemplate.getSpeedOnRide(MoveType.RUN), petLevelTemplate.getSpeedOnRide(MoveType.SLOW_SWIM), petLevelTemplate.getSpeedOnRide(MoveType.FAST_SWIM), petLevelTemplate.getSpeedOnRide(MoveType.SLOW_FLY), petLevelTemplate.getSpeedOnRide(MoveType.FAST_FLY), typeId, level))
//								_log.warn("Error: set - typeId: " + typeId + " lvl: " + level);
//						}
//					}
//
//				}
...
//		}
				...
				for (Node p = d.getFirstChild(); p != null; p = p.getNextSibling())
				{
					...
					else if (p.getNodeName().equals("skills"))
					{
						for (Node s = p.getFirstChild(); s != null; s = s.getNextSibling())
						{
							...
							{
								final int minLvl = s.getAttributes().getNamedItem("minLvl") != null ? Integer.parseInt(s.getAttributes().getNamedItem("minLvl").getNodeValue()) : 0;
								final int skillId = Integer.parseInt(s.getAttributes().getNamedItem("skillId").getNodeValue());
								final int skillLvl = Integer.parseInt(s.getAttributes().getNamedItem("skillLvl").getNodeValue());
								final double hpPercent = s.getAttributes().getNamedItem("hpPencent") != null ? Double.parseDouble(s.getAttributes().getNamedItem("hpPencent").getNodeValue()) : 0;
								data.addNewSkill(skillId, skillLvl, minLvl, hpPercent);
							}
						}
					}
	...
	{
		for (final PetData data : _pets.values())
		{
	...
	public PetData getPetData(int petId)
	{
		...
		{
			info("Missing pet data for npcid: " + petId);
		}
	...
	public static int[] getPetItemsByNpc(int npcId)
	{
		switch (npcId)
		{
			...
			case 16053 :
				return new int[]
				{
				        10311, 10312, 10313, 13020, 13019, 13017, 13018, 13550, 14062, 14061, 13551
				};
			default :
				return new int[]
				{
