...
public final class FakeArmorParser extends DocumentParser
{
	...
	public void load()
	{
		...
		parseDatapackFile("config/mods/fakes/armorSets.xml");
		
		info("Loaded " + (_phantomNgradeItemsList.size() + _phantomDgradeItemsList.size() + _phantomCgradeItemsList.size() + _phantomBgradeItemsList.size() + _phantomAgradeItemsList.size() + _phantomSgradeItemsList.size() + _phantomS80gradeItemsList.size() + _phantomS84gradeItemsList.size()) + " fake player sets.");
	}
