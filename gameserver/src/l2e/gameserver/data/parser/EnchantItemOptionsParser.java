...
public class EnchantItemOptionsParser extends DocumentParser
{
	...
	protected void parseDocument()
	{
		...
		for (Node n = getCurrentDocument().getFirstChild(); n != null; n = n.getNextSibling())
		{
			if ("list".equalsIgnoreCase(n.getNodeName()))
			{
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					...
					{
						final int itemId = parseInt(d.getAttributes(), "id");
						...
						for (Node cd = d.getFirstChild(); cd != null; cd = cd.getNextSibling())
						{
							...
							{
								final int minLevel = cd.getAttributes().getNamedItem("minLevel") != null ? parseInt(cd.getAttributes(), "minLevel") : 0;
								final int maxLevel = cd.getAttributes().getNamedItem("maxLevel") != null ? parseInt(cd.getAttributes(), "maxLevel") : 0;
								if (maxLevel != 0)
								{
									for (int level = minLevel; level <= maxLevel; ++level)
									{
										op = new EnchantOptions(level);
		...
		info("Loaded: " + _data.size() + " items and " + counter + " options.");
	}
