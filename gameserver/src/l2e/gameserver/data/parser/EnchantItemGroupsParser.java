...
public final class EnchantItemGroupsParser extends DocumentParser
{
	...
	public synchronized void load()
	{
		...
		info("Loaded: " + _itemGroups.size() + " items and " + _scrollGroups.size() + " scrolls group template.");
	}
					...
					if ("enchantRateGroup".equalsIgnoreCase(d.getNodeName()))
					{
						...
						for (Node cd = d.getFirstChild(); cd != null; cd = cd.getNextSibling())
						{
							if ("current".equalsIgnoreCase(cd.getNodeName()))
							{
								...
								final boolean enchantAnnounce = parseBoolean(cd.getAttributes(), "enchantAnnounce", false);
								int min = -1;
								int max = 0;
								if (range.contains("-"))
								{
								...
								if ((min >= 0) && (max >= 0))
								{
									group.addChance(new RangeChanceHolder(min, max, chance, enchantAnnounce));
								}
							}
						}
