...
public final class EnchantSkillLearn
{
	...
	{
		final int enchantType = EnchantSkillLearn.getEnchantRoute(level);
		if ((enchantType < 1) || !_enchantRoutes.containsKey(enchantType))
		{
			return false;
		}
		final int index = EnchantSkillLearn.getEnchantIndex(level);
	...
	public EnchantSkillsHolder getEnchantSkillsHolder(int level)
	{
		...
		final int index = EnchantSkillLearn.getEnchantIndex(level);
		final EnchantSkillGroup group = EnchantSkillGroupsParser.getInstance().getEnchantSkillGroupById(_enchantRoutes.get(enchantType));
