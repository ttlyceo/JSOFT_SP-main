		...
		else if (actualCommand.equalsIgnoreCase("supply"))
		{
			if (isMyLord(player))
			{
				...
				else
				{
					...
					if (level > 0)
					{
						final NpcTemplate BoxTemplate = NpcsParser.getInstance().getTemplate(SUPPLY_BOX_IDS[level - 1]);
						final MonsterInstance box = new MonsterInstance(IdFactory.getInstance().getNextId(), BoxTemplate);
