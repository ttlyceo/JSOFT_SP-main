		...
		else if (condition == COND_OWNER)
		{
			...
			else if (actualCommand.equalsIgnoreCase("receive_report"))
			{
				...
				else
				{
					...
					final var castle = getFort().getContractedCastle();
					if (castle != null && castle.getTemplate() != null)
					{
						html.replace("%castle%", castle.getName(player.getLang()));
					}
					else
					{
						html.replace("%castle%", "");
			...
			else if (actualCommand.equalsIgnoreCase("operate_door"))
			{
				if ((player.getClanPrivileges() & Clan.CP_CS_OPEN_DOOR) == Clan.CP_CS_OPEN_DOOR)
				{
					...
					{
						final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
						html.setFile(player, player.getLang(), "data/html/fortress/" + getTemplate().getId() + "-d.htm");
						html.replace("%objectId%", String.valueOf(getObjectId()));
				...
				if ((player.getClanPrivileges() & Clan.CP_CL_VIEW_WAREHOUSE) == Clan.CP_CL_VIEW_WAREHOUSE)
				{
					...
					else if (val.equalsIgnoreCase("withdraw"))
					{
						if (Config.ENABLE_WAREHOUSESORTING_CLAN)
						{
							...
							final String htmContent = HtmCache.getInstance().getHtm(player, player.getLang(), htmFile);
							if (htmContent != null)
							{
			...
			else if (actualCommand.equalsIgnoreCase("functions"))
			{
				if (val.equalsIgnoreCase("tele"))
				{
					...
					{
						html.setFile(player, player.getLang(), "data/html/fortress/" + getId() + "-t" + getFort().getFunction(Fort.FUNC_TELEPORT).getLvl() + ".htm");
					}
					sendHtmlMessage(player, html);
				}
				else if (val.equalsIgnoreCase("support"))
				{
					...
					{
						html.setFile(player, player.getLang(), "data/html/fortress/support" + getFort().getFunction(Fort.FUNC_SUPPORT).getLvl() + ".htm");
	...
	private void doTeleport(Player player, int val)
	{
		...
		if (list != null)
		{
			if (player.destroyItemByItemId("Teleport", list.getId(), list.getPrice(), this, true))
			{
				if (Config.DEBUG)
				{
					_log.warn("Teleporting player " + player.getName(null) + " for Fortress to new location: " + list.getLocation().toString());
				}
