...
public final class Transform implements IIdentifiable
{
	...
	public double getCollisionHeight(Player player)
	{
		...
		return template != null ? template.getCollisionHeight() : player.getAppearance().getSex() ? player.getBaseTemplate().getFCollisionHeightFemale() : player.getBaseTemplate().getfCollisionHeight();
	}

	public double getCollisionRadius(Player player)
	{
		final TransformTemplate template = getTemplate(player);
		return template != null ? template.getCollisionRadius() : player.getAppearance().getSex() ? player.getBaseTemplate().getFCollisionRadiusFemale() : player.getBaseTemplate().getfCollisionRadius();
	}
	...
	public void onTransform(Player player)
	{
		...
		if (template != null)
		{
			...
			{
				if (Config.ALLOW_MUTIPROFF_SYSTEM && SkillTreesParser.getInstance().isAllowMultiSkill(holder.getId()))
				{
					if (player.getKnownSkill(holder.getId()) == null)
					{
					...
					{
						player.addTransformNotCheckSkill(holder.getId());
					}
				}
				else
				{
			...
			for (final SkillLearn skill : SkillTreesParser.getInstance().getCollectSkillTree().values())
			{
			...
			if (!template.getAdditionalItems().isEmpty())
			{
				...
				if (!allowed.isEmpty())
				{
					player.getInventory().setInventoryBlock(allowed, 1);
				}

				if (!notAllowed.isEmpty())
				{
					player.getInventory().setInventoryBlock(notAllowed, 2);
				}
			}
			
			player.getFarmSystem().checkSkillsAtTransformation();
	...
	public void onUntransform(Player player)
	{
		...
		if (template != null)
		{
			...
			player.getFarmSystem().checkSkillsAtRemoveTransformation(player.getTransformSkills());
			...
			if (!template.getSkills().isEmpty())
			{
				for (final SkillHolder holder : template.getSkills())
				{
					...
					if (!SkillTreesParser.getInstance().isSkillAllowed(player, skill) && !player.getTransformNotCheckSkills().contains(holder.getId()))
					{
			...
			if (!template.getAdditionalSkills().isEmpty())
			{
				...
				{
					final Skill skill = holder.getSkill();
					if ((player.getLevel() >= holder.getMinLevel()) && !SkillTreesParser.getInstance().isSkillAllowed(player, skill) && !player.getTransformNotCheckSkills().contains(holder.getId()))
					{
						player.removeSkill(skill, false, skill.isPassive());
					}
				}
