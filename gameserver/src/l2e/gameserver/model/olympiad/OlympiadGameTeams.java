...
public class OlympiadGameTeams extends AbstractOlympiadGame
{
	...
	protected OlympiadGameTeams(int id, Participant[] teamOne, Participant[] teamTwo, boolean isTraining)
	{
		super(id);
		...
		_currentInvites = 0;
		_confirmInvites = (_teamOne.length + _teamTwo.length);
	...
	protected static final Participant[][] createListOfParticipants(List<List<Integer>> list)
	{
		...
		while (list.size() > 1)
		{
			...
			for (final int objectId : teamOne)
			{
			...
			for (final int objectId : teamTwo)
			{
				player = GameObjectsStorage.getPlayer(objectId);
				if ((player == null) || !player.isOnline())
				{
	...
	protected static OlympiadGameTeams createGame(int id, List<List<Integer>> list)
	{
		...
		return new OlympiadGameTeams(id, teams[0], teams[1], Olympiad.getInstance().isTrainingPeriod());
	}
	...
	@Override
	protected int[][] getLoseReward()
	{
		return Config.ALT_OLY_TEAM_LOSE_REWARD;
	}
	...
	public final boolean containsHwidParticipant(Player player)
	{
		for (int i = _teamOneSize; --i >= 0;)
		{
			if (_teamOne[i].getPlayer().getHWID().equalsIgnoreCase(player.getHWID()))
			{
				return true;
			}
		}
		...
		for (int i = _teamTwoSize; --i >= 0;)
		{
			if (_teamTwo[i].getPlayer().getHWID().equalsIgnoreCase(player.getHWID()))
			{
				return true;
			}
		}
	...
	protected final void confirmDlgInvite()
	{
		...
		for (int i = 0; i < _teamOneSize; i++)
		{
			if (_teamOne[i].getPlayer() != null)
			{
				_teamOne[i].getPlayer().sendConfirmDlg(new AnswerClick(_teamOne[i].getPlayer()), 10000, new ServerMessage("Olympiad.INVITE", _teamOne[i].getPlayer().getLang()).toString());
			}
		}
		
		for (int i = 0; i < _teamTwoSize; i++)
		{
			if (_teamTwo[i].getPlayer() != null)
			{
				_teamTwo[i].getPlayer().sendConfirmDlg(new AnswerClick(_teamTwo[i].getPlayer()), 10000, new ServerMessage("Olympiad.INVITE", _teamTwo[i].getPlayer().getLang()).toString());
			}
		}
	}
	...
	public void healPlayers()
	{
		...
		for (final Participant plr : _teamOne)
		{
			final Player player = plr.getPlayer();
			player.setCurrentCp(player.getMaxCp());
			player.setCurrentHp(player.getMaxHp());
			player.setCurrentMp(player.getMaxMp());
			if (Config.OLY_PRINT_CLASS_OPPONENT)
			{
				enMsg1 += ", " + Util.className("en", player.getClassId().getId()) + "";
				ruMsg1 += ", " + Util.className("ru", player.getClassId().getId()) + "";
			}
		}

		for (final Participant plr : _teamTwo)
		{
			final Player player = plr.getPlayer();
			player.setCurrentCp(player.getMaxCp());
			player.setCurrentHp(player.getMaxHp());
			...
			if (Config.OLY_PRINT_CLASS_OPPONENT)
			{
				enMsg2 += ", " + Util.className("en", player.getClassId().getId()) + "";
				ruMsg2 += ", " + Util.className("ru", player.getClassId().getId()) + "";
			}
		}
		...
		if (Config.OLY_PRINT_CLASS_OPPONENT)
		{
			for (final Participant plr : _teamOne)
			{
				final ServerMessage msg = new ServerMessage("Olympiad.OPPONENTS", plr.getPlayer().getLang());
				msg.add(plr.getPlayer().getLang().equalsIgnoreCase("ru") ? ruMsg2 : enMsg2);
				plr.getPlayer().sendPacket(new ExShowScreenMessage(msg.toString(), 5000, ExShowScreenMessage.TOP_CENTER, false));
				plr.getPlayer().sendMessage(msg.toString());
			}
			...
			try
			{
				if (_teamOneDefaulted)
				{
					for (int i = _teamOneSize; --i >= 0;)
					{
						...
						if (_isTraining)
						{
							list1.add(new OlympiadInfo(par.getName(), par.getClanName(), par.getClanId(), par.getBaseClass(), _damageT1, 0, 0));
						}
						else
						{
							points = par.getStats().getInteger(POINTS) / getDivider();
							final int val = Math.min(par.getStats().getInteger(POINTS) / 3, Config.ALT_OLY_MAX_POINTS);
				...
				if (_teamTwoDefaulted)
				{
					for (int i = _teamTwoSize; --i >= 0;)
					{
						...
						if (_isTraining)
						{
							list2.add(new OlympiadInfo(par.getName(), par.getClanName(), par.getClanId(), par.getBaseClass(), _damageT2, 0, 0));
						}
						else
						{
							points = par.getStats().getInteger(POINTS) / getDivider();
		...
		if (tOneCrash || tTwoCrash)
		{
			try
			{
				if (tTwoCrash && !tOneCrash)
				{
					...
					{
						final var msg = new ServerMessage("Olympiad.TEAM_ANNOUNCE", true);
						msg.add(_teamOne[0].getName());
						msg.add(_teamTwo[0].getName());
						msg.add(_teamOne[0].getName());
		...
		try
		{
			...
			if (((teamTwoHp == 0) && (teamOneHp != 0)) || ((_damageT1 < _damageT2) && (teamTwoHp != 0) && (teamOneHp != 0)))
			{
				...
				for (int i = 0; i < _teamTwoSize; i++)
				{
					...
					else
					{
						...
						removePointsFromParticipant(par, points);
						list2.add(new OlympiadInfo(par.getName(), par.getClanName(), par.getClanId(), par.getBaseClass(), _damageT2, par.getStats().getInteger(POINTS) - points, -points));
						RewardManager.getInstance().checkOlyRewards(null, par.getPlayer());
					}
				}
				...
				if (!_isTraining)
				{
					...
					{
						rewardParticipant(_teamOne[i].getPlayer(), getReward());
						RewardManager.getInstance().checkOlyRewards(_teamOne[i].getPlayer(), null);
					}
					...
					if (getLoseReward() != null)
					{
						for (int i = 0; i < _teamTwoSize; i++)
						{
							rewardParticipant(_teamTwo[i].getPlayer(), getLoseReward());
						}
					}
				}
			}
			else if (((teamOneHp == 0) && (teamTwoHp != 0)) || ((_damageT2 < _damageT1) && (teamOneHp != 0) && (teamTwoHp != 0)))
			{
				...
				if (!_isTraining)
				{
					...
					{
						rewardParticipant(_teamTwo[i].getPlayer(), getReward());
						RewardManager.getInstance().checkOlyRewards(_teamTwo[i].getPlayer(), null);
					}
					...
					if (getLoseReward() != null)
					{
						for (int i = 0; i < _teamOneSize; i++)
						{
							rewardParticipant(_teamOne[i].getPlayer(), getLoseReward());
						}
					}
				}
			}
	...
	public final String[] getPlayerNames()
	{
		return new String[]
		{
		        _teamOne[0].getName(), _teamTwo[0].getName()
		};
	}
			...
			for (int i = _teamOneSize; --i >= 0;)
			{
				...
				if (reason != null)
				{
					...
					if (!_teamOneDefaulted)
					{
						...
						for (final Participant t : _teamTwo)
						{
							if (t.getPlayer() != null)
							{
			...
			for (int i = _teamTwoSize; --i >= 0;)
			{
				...
				if (reason != null)
				{
					...
					if (!_teamTwoDefaulted)
					{
						...
						for (final Participant t : _teamOne)
						{
							if (t.getPlayer() != null)
							{
								t.getPlayer().sendPacket(reason);
