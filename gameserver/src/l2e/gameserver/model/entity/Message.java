...
public class Message
{
	...
	public Message(ResultSet rset) throws SQLException
	{
		...
		_type = SenderType.values()[rset.getInt("sendBySystem")];
	...
	public Message(int receiverId, String subject, String content, SenderType sendBySystem)
	{
		_messageId = IdFactory.getInstance().getNextId();
		_senderId = -1;
		_receiverId = receiverId;
	...
	public Message(Message msg)
	{
		...
		_unloadTask = ThreadPoolManager.getInstance().schedule(new AttachmentsUnloadTask(this), UNLOAD_ATTACHMENTS_INTERVAL + Rnd.get(UNLOAD_ATTACHMENTS_INTERVAL));
	}
	...
	public static final PreparedStatement getStatement(Message msg, Connection con) throws SQLException
	{
		...
		stmt.setInt(8, msg._hasAttachments ? 1 : 0);
		stmt.setInt(9, msg._unread ? 1 : 0);
		stmt.setInt(10, msg._deletedBySender ? 1 : 0);
		stmt.setInt(11, msg._deletedByReceiver ? 1 : 0);
		stmt.setInt(12, msg._type.ordinal());
		stmt.setInt(13, msg._returned ? 1 : 0);
	...
	public final String getSenderName()
	{
		if (_senderName == null)
		{
			switch (_type)
			{
				case NORMAL :
				{
					_senderName = CharNameHolder.getInstance().getNameById(_senderId);
					if (_senderName == null)
					{
						_senderName = "";
					}
	...
	public final String getReceiverName()
	{
		...
		{
			_receiverName = CharNameHolder.getInstance().getNameById(_receiverId);
			if (_receiverName == null)
			{
				_receiverName = "";
			}
	...
	{
		return _type == SenderType.NORMAL && _hasAttachments && !_returned;
	}
