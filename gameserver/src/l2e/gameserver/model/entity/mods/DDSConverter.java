...
{
	private static final Logger _log = LoggerFactory.getLogger(DDSConverter.class);
	...
	public static ByteBuffer convertToDDS(File file)
	{
		if (file == null)
		{
			final String s = "nullValue.FileIsNull";
			_log.warn(s, new IllegalArgumentException(s));
			return null;
		}
		if (!file.exists() || !file.canRead())
		{
			final String s1 = "DDSConverter.NoFileOrNoPermission";
			_log.warn(s1, new IllegalArgumentException(s1));
			return null;
		}
		...
		{
			_log.warn("Error while reading Image that needs to be DDSConverted", e);
		}
		if (bufferedimage == null)
		{
			return null;
	...
	protected static int[] determineExtremeColors(Color acolor[])
	{
		...
		for (int j = 0; j < (acolor.length - 1); j++)
		{
			...
			{
				final int l = distance(acolor[j], acolor[k]);
				if (l > i)
				{
					i = l;
					ai[0] = j;
	...
	protected static long computeBitMask(Color acolor[], int ai[])
	{
		...
		for (int i = 0; i < acolor.length; i++)
		{
			...
			{
				final int j1 = distance(acolor[i], acolor1[i1]);
				if (j1 < j)
				{
					j = j1;
					k = i1;
