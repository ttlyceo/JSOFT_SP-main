...
public class PreparedEntry extends Entry
{
	...
	public PreparedEntry(Entry template, ItemInstance item, boolean applyTaxes, boolean maintainEnchantment, double taxRate)
	{
		...
		for (final Ingredient ing : template.getIngredients())
		{
			if (ing.getId() == ADENA_ID)
			{
				if (ing.isTaxIngredient())
				{
					if (applyTaxes)
					{
						_taxAmount += Math.round(ing.getCount() * taxRate);
					}
				}
				else
				{
			...
			else if (maintainEnchantment && (item != null) && ing.isArmorOrWeapon())
			{
				...
				newIngredient.setCount(item.getCount());//
		...
		if (adenaAmount > 0)
		{
			_ingredients.add(new Ingredient(ADENA_ID, adenaAmount, 0, -1, null, null, false, false, false, 0, 0));
		}

		_products = new ArrayList<>(template.getProducts().size());
		for (final Ingredient ing : template.getProducts())
		{
			if (!ing.isStackable())
			{
				_stackable = false;
