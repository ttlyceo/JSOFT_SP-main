...
public final class Product
{
	...
	public boolean decreaseCount(long val)
	{
		...
		{
			_restockTask = ThreadPoolManager.getInstance().schedule(new RestockTask(), getRestockDelay());
		}
	...
	public void restartRestockTask(long nextRestockTime)
	{
		...
		if (remainTime > 0)
		{
			_restockTask = ThreadPoolManager.getInstance().schedule(new RestockTask(), remainTime);
		}
		else
		{
			restock();
	...
	private void save()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("INSERT INTO `buylists`(`buylist_id`, `item_id`, `count`, `next_restock_time`) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE `count` = ?, `next_restock_time` = ?");
