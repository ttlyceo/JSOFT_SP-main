...
public class CursedWeapon
{
	...
	public CursedWeapon(int itemId, int skillId, String name)
	{
		...
		_itemId = itemId;
		_skillId = skillId;
		_skillMaxLevel = SkillsParser.getInstance().getMaxLevel(_skillId);
	}
	...
	private void dropIt(Attackable attackable, Player player, Creature killer, boolean fromMonster)
	{
		...
		if (fromMonster)
		{
			_item = attackable.dropSingleItem(player, _itemId, 1);
			_item.setDropTime(0);
			
			final var packet = new ExRedSky(10);
			final var eq = new EarthQuake(player.getX(), player.getY(), player.getZ(), 14, 3);
			GameObjectsStorage.getPlayers().stream().filter(p -> p != null && p.isOnline()).forEach(p -> p.sendPacket(packet, eq));
		}
		else
		{
	...
	public void cursedOnLogin()
	{
		...
		final SystemMessage msg = SystemMessage.getSystemMessage(SystemMessageId.S2_OWNER_HAS_LOGGED_INTO_THE_S1_REGION);
		...
		final SystemMessage msg2 = SystemMessage.getSystemMessage(SystemMessageId.S2_MINUTE_OF_USAGE_TIME_ARE_LEFT_FOR_S1);
	...
	public void removeSkill()
	{
		...
		_player.removeSkill(SkillsParser.FrequentSkill.VOID_BURST.getSkill().getId());
		_player.removeSkill(SkillsParser.FrequentSkill.VOID_FLOW.getSkill().getId());
		_player.untransform();
		_player.sendSkillList(false);
	}
	...
	public void reActivate()
	{
		...
		{
			_removeTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new RemoveTask(), _durationLost * 12000L, _durationLost * 12000L);
		}
	...
	public void saveData()
	{
		...
		{
			_log.warn("CursedWeapon: Failed to save data.", e);
		}
		finally
		{
			DbUtils.closeQuietly(con, statement);
	...
	public void goTo(Player player)
	{
		...
		{
			player.teleToLocation(_player.getX(), _player.getY(), _player.getZ() + 20, true, _player.getReflection());
		}
		else if (_isDropped && (_item != null))
		{
			player.teleToLocation(_item.getX(), _item.getY(), _item.getZ() + 20, true, _item.getReflection());
		}
		else
		{
