...
{
	private static final Logger _log = LoggerFactory.getLogger(ChanceCondition.class);
	...
	{
		ON_HIT(1), ON_CRIT(2), ON_CAST(4), ON_PHYSICAL(8), ON_MAGIC(16), ON_MAGIC_GOOD(32), ON_MAGIC_OFFENSIVE(64), ON_ATTACKED(128), ON_ATTACKED_HIT(256), ON_ATTACKED_CRIT(512), ON_HIT_BY_SKILL(1024), ON_HIT_BY_OFFENSIVE_SKILL(2048), ON_HIT_BY_GOOD_MAGIC(4096), ON_EVADED_HIT(8192), ON_START(16384), ON_ACTION_TIME(32768), ON_EXIT(65536);
	...
	public static ChanceCondition parse(StatsSet set)
	{
		try
		{
			...
			final String activationSkills = set.getString("activationSkills", null);
	...
	public static final byte[] parseElements(String list)
	{
		...
		final String[] valuesSplit = list.split(",");
		final byte[] elements = new byte[valuesSplit.length];
	...
	public static final int[] parseActivationSkills(String list)
	{
		...
		final String[] valuesSplit = list.split(",");
		final int[] skillIds = new int[valuesSplit.length];
		for (int i = 0; i < valuesSplit.length; i++)
		{
	...
	public boolean trigger(int event, int damage, byte element, boolean playable, Skill skill)
	{
		...
		return _triggerType.check(event) && (_chance < 0 || Rnd.chance(_chance));
	}
