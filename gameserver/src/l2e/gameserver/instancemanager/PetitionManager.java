...
public final class PetitionManager extends LoggerObject
{
	...
	public void clearCompletedPetitions()
	{
		...
		info("Completed petition data cleared. " + numPetitions + " petition(s) removed.");
	}
	...
	public void clearPendingPetitions()
	{
		...
		info("Pending petition queue cleared. " + numPetitions + " petition(s) removed.");
	}
	...
	public boolean acceptPetition(Player respondingAdmin, int petitionId)
	{
		...
		sm.addString(currPetition.getPetitioner().getName(null));
		currPetition.sendResponderPacket(sm);

		currPetition.getPetitioner().setLastPetitionGmName(currPetition.getResponder().getName(null));
		return true;
	}
	...
	public boolean sendActivePetitionMessage(Player player, String messageText)
	{
		...
		for (final Petition currPetition : getPendingPetitions().values())
		{
			...
			if ((currPetition.getPetitioner() != null) && (currPetition.getPetitioner().getObjectId() == player.getObjectId()))
			{
				cs = new CreatureSay(player.getObjectId(), Say2.PETITION_PLAYER, player.getName(null), messageText);
			...
			if ((currPetition.getResponder() != null) && (currPetition.getResponder().getObjectId() == player.getObjectId()))
			{
				cs = new CreatureSay(player.getObjectId(), Say2.PETITION_GM, player.getName(null), messageText);
				currPetition.addLogMessage(cs);
	...
	public void sendPendingPetitionList(Player activeChar)
	{
		...
		for (final Petition currPetition : getPendingPetitions().values())
		{
			...
			StringUtil.append(htmlContent, "</td><td width=\"140\" align=right><font color=\"", (currPetition.getPetitioner().isOnline() ? "00FF00" : "999999"), "\">", currPetition.getPetitioner().getName(null), "</font></td></tr>");
			...
			{
				htmlContent.append("<font color=\"" + (currPetition.getResponder().isOnline() ? "00FF00" : "999999") + "\">" + currPetition.getResponder().getName(null) + "</font>");
			}
	...
	public int submitPetition(Player petitioner, String petitionText, int petitionType)
	{
		...
		final String msgContent = petitioner.getName(null) + " has submitted a new petition.";
	...
	public void viewPetition(Player activeChar, int petitionId)
	{
		...
		html.replace("%petitioner%", currPetition.getPetitioner().getName(null));
