...
public class Lottery extends LoggerObject
{
	...
	public void increasePrize(long count)
	{
		...
		{
			final var statement = con.prepareStatement(UPDATE_PRICE);
		...
		{
			warn("Could not increase current lottery prize: " + e.getMessage(), e);
		}
	}
	...
	private class StartLotteryTask implements Runnable
	{
		@Override
		public void run()
		{
			...
			try (
			    var con = DatabaseFactory.getInstance().getConnection(); Statement statement = con.createStatement(); var rset = statement.executeQuery(SELECT_LAST_LOTTERY))
			{
				if (rset.next())
				{
			...
			{
				warn("Could not restore lottery data: " + e.getMessage(), e);
			}

			if (Config.DEBUG)
			{
				info("Starting ticket sell for lottery #" + getId() + ".");
			}
			_isSellingTickets = true;
			_isStarted = true;
			...
			try (
			    var con = DatabaseFactory.getInstance().getConnection(); var statement = con.prepareStatement(INSERT_LOTTERY))
			{
			...
			{
				warn("Could not store new lottery data: " + e.getMessage(), e);
			}
		}
	}
	...
	private class stopSellingTickets implements Runnable
	{
		...
		public void run()
		{
			if (Config.DEBUG)
			{
				info("Stopping ticket sell for lottery #" + getId() + ".");
			}
			_isSellingTickets = false;
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			if (Config.DEBUG)
			{
				info("The lucky numbers are " + luckynums[0] + ", " + luckynums[1] + ", " + luckynums[2] + ", " + luckynums[3] + ", " + luckynums[4] + ".");
			}
			...
			if (Config.DEBUG)
			{
				info("Encoded lucky numbers are " + enchant + ", " + type2);
			}
			...
			try (
			    var con = DatabaseFactory.getInstance().getConnection(); var statement = con.prepareStatement(SELECT_LOTTERY_ITEM))
			{
			...
			{
				warn("Could restore lottery data: " + e.getMessage(), e);
			}
			...
			if (Config.DEBUG)
			{
				info("" + count1 + " players with all FIVE numbers each win " + prize1 + ".");
				info("" + count2 + " players with FOUR numbers each win " + prize2 + ".");
				info("" + count3 + " players with THREE numbers each win " + prize3 + ".");
				info("" + count4 + " players with ONE or TWO numbers each win " + prize4 + ".");
			}

			final long newprize = getPrize() - (prize1 + prize2 + prize3 + prize4);
			if (Config.DEBUG)
			{
				info("Jackpot for next lottery is " + newprize + ".");
			}
			...
			try (
			    var con = DatabaseFactory.getInstance().getConnection(); var statement = con.prepareStatement(UPDATE_LOTTERY))
			{
				statement.setLong(1, getPrize());
			...
			{
				warn("Could not store finished lottery data: " + e.getMessage(), e);
			}

			ThreadPoolManager.getInstance().schedule(new StartLotteryTask(), MINUTE);
			_number++;
	...
	public long[] checkTicket(int id, int enchant, int type2)
	{
		...
		try (
		    var con = DatabaseFactory.getInstance().getConnection(); var statement = con.prepareStatement(SELECT_LOTTERY_TICKET))
		{
			    ...
			    var rset = statement.executeQuery())
			{
				if (rset.next())
				{
					...
					if (Config.DEBUG)
					{
						warn("count: " + count + ", id: " + id + ", enchant: " + enchant + ", type2: " + type2);
					}
				}
			}
		}
		...
		{
			warn("Could not check lottery ticket #" + id + ": " + e.getMessage(), e);
		}
		return res;
	}
