...
public final class FourSepulchersChangeAttackTimeTask implements Runnable
{
	...
	public void run()
	{
		...
		if (manager.isFirstTimeRun())
		{
			for (double min = Calendar.getInstance().get(Calendar.MINUTE); min < manager.getCycleMin(); min++)
			{
				if ((min % 5) == 0)
				{
					...
					ThreadPoolManager.getInstance().schedule(new FourSepulchersManagerSayTask(), inter.getTimeInMillis() - Calendar.getInstance().getTimeInMillis());
					break;
				}
			}
		}
		...
		{
			ThreadPoolManager.getInstance().schedule(new FourSepulchersManagerSayTask(), 5 * 60400);
		}
