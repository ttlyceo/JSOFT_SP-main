...
public class CallPc extends Effect
{
	...
	public boolean onStart()
	{
		...
		final Player activeChar = getEffector().getActingPlayer();
		if (checkSummonTargetStatus(target, activeChar))
		{
			...
			target.addScript(new SummonRequestHolder(activeChar, getSkill(), false));
	...
	public static boolean checkSummonTargetStatus(Player target, Player activeChar)
	{
		...
		if (target.isInStoreMode() || target.isSellingBuffs())
		{
			final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.C1_CURRENTLY_TRADING_OR_OPERATING_PRIVATE_STORE_AND_CANNOT_BE_SUMMONED);
			sm.addPcName(target);
			activeChar.sendPacket(sm);
			return false;
		}
		...
		for (final AbstractFightEvent e : target.getFightEvents())
		{
		...
		var e = target.getPartyTournament();
		if (e != null && !e.canUseEscape(target))
		{
			activeChar.sendPacket(SystemMessageId.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING);
			return false;
		...
		{
			final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.C1_STATE_FORBIDS_SUMMONING);
		...
		{
			final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.C1_IN_SUMMON_BLOCKING_AREA);
		...
		{
			final Reflection summonerInstance = ReflectionManager.getInstance().getReflection(activeChar.getReflectionId());
