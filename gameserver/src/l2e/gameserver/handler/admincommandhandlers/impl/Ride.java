...
public class Ride implements IAdminCommandHandler
{
	...
	{
	        "admin_ride_horse", "admin_ride_bike", "admin_ride_wyvern", "admin_ride_strider", "admin_unride_wyvern", "admin_unride_strider", "admin_unride", "admin_ride_wolf", "admin_unride_wolf",
	};
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		if (command.startsWith("admin_ride"))
		{
			...
			else if (command.startsWith("admin_ride_horse")) // handled using
			                                                 // transformation
			{
				...
				{
					TransformParser.getInstance().transformPlayer(PURPLE_MANED_HORSE_TRANSFORMATION_ID, player);
				}

				return true;
			}
			else if (command.startsWith("admin_ride_bike")) // handled using
			                                                // transformation
			{
				if (player.isTransformed() || player.isInStance())
				{
				...
				{
					TransformParser.getInstance().transformPlayer(JET_BIKE_TRANSFORMATION_ID, player);
				}
