...
public class FightCalculator implements IAdminCommandHandler
{
	...
	private void handleStart(String params, Player activeChar)
	{
		...
		else if ((lvl1 != 0) && (npc1 == null))
		{
			...
			{
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_fight_calculator lvl1 ", String.valueOf(lvl1), " lvl2 ", String.valueOf(lvl2), " mid1 ", String.valueOf(n.getId()), " mid2 ", String.valueOf(mid2), "\">", n.getName(activeChar.getLang()), "</a></td></tr>");
			}
		...
		{
			final List<NpcTemplate> npcs = NpcsParser.getInstance().getAllOfLevel(lvl2);
			final StringBuilder sb = StringUtil.startAppend(50 + (npcs.size() * 200), "<html><title>Select second mob to fight</title>" + "<body><table>");
			
			for (final NpcTemplate n : npcs)
			{
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_fight_calculator lvl1 ", String.valueOf(lvl1), " lvl2 ", String.valueOf(lvl2), " mid1 ", String.valueOf(mid1), " mid2 ", String.valueOf(n.getId()), "\">", n.getName(activeChar.getLang()), "</a></td></tr>");
			}
	...
	private void handleShow(String params, Player activeChar)
	{
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			final boolean _crit1 = Rnd.chance(Formulas.calcCrit(npc1, npc2, null, false));
			if (_crit1)
			{
				crit1++;
			}
			...
			_patk1 += (1 + (Rnd.get() * npc1.getRandomDamage() * 2 - npc1.getRandomDamage()) / 100);
			patk1 += _patk1;
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			final boolean _crit2 = Rnd.chance(Formulas.calcCrit(npc2, npc1, null, false));
			if (_crit2)
			{
				crit2++;
			}
			...
			_patk2 *= (1 + (Rnd.get() * npc2.getRandomDamage() * 2 - npc2.getRandomDamage()) / 100);
			patk2 += _patk2;
		...
		{
			StringUtil.append(replyMSG, "<tr><td width=140>Parameter</td><td width=70>", ((NpcTemplate) npc1.getTemplate()).getName(activeChar.getLang()), "</td><td width=70>", ((NpcTemplate) npc2.getTemplate()).getName(activeChar.getLang()), "</td></tr>");
		}
		...
		{
			StringUtil.append(replyMSG, "<button value=\"Retry\" action=\"bypass -h admin_fight_calculator_show ", String.valueOf(((NpcTemplate) npc1.getTemplate()).getId()), " ", String.valueOf(((NpcTemplate) npc2.getTemplate()).getId()), "\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		}
