...
public class MobGroups implements IAdminCommandHandler
{
	...
	private void returnToChar(String command, Player activeChar)
	{
		...
		final MobGroup group = MobGroupData.getInstance().getGroup(groupId);
		if (group == null)
		{
	...
	private void createGroup(String command, Player activeChar)
	{
		...
		final NpcTemplate template = NpcsParser.getInstance().getTemplate(templateId);
	...
	{
		activeChar.broadcastPacket(1500, new MagicSkillUse(activeChar, 1008, 1, 4000, 0));
	...
	private void attackGrp(String command, Player activeChar)
	{
		...
		final MobGroup othGroup = MobGroupData.getInstance().getGroup(othGroupId);
	...
	private void teleportGroup(String command, Player activeChar)
	{
		...
		try
		{
			...
			{
				targetPlayer = GameObjectsStorage.getPlayer(targetPlayerStr);
			}
	...
	{
		final MobGroup[] mobGroupList = MobGroupData.getInstance().getGroups();
