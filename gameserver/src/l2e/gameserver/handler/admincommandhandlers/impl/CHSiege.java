...
public final class CHSiege implements IAdminCommandHandler
{
	...
	{
	        "admin_chsiege_siegablehall", "admin_chsiege_startSiege", "admin_chsiege_endsSiege", "admin_chsiege_setSiegeDate", "admin_chsiege_addAttacker", "admin_chsiege_removeAttacker", "admin_chsiege_clearAttackers", "admin_chsiege_listAttackers", "admin_chsiege_forwardSiege"
	};
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (split[0].equals(COMMANDS[4]))
		{
			...
			if (split.length < 3)
			{
				final GameObject rawTarget = activeChar.getTarget();
				Player target = null;
				if (rawTarget == null)
				{
			...
			{
				final Clan rawClan = ClanHolder.getInstance().getClanByName(split[2]);
				if (rawClan == null)
				{
	...
	private void sendSiegableHallPage(Player activeChar, String hallId, SiegableHall hall)
	{
		...
		msg.replace("%clanhallName%", Util.clanHallName(activeChar, parseInt(hallId)));
