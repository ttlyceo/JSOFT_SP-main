...
public class PledgeShowMemberListAll extends GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		for (final SubPledge subPledge : _clan.getAllSubPledges())
		{
			_activeChar.sendPacket(new PledgeReceiveSubPledgeCreated(subPledge, _clan));
		}

		for (final ClanMember m : _members)
		{
	...
	void writePledge(int mainOrSubpledge)
	{
		...
		for (final ClanMember m : _members)
		{
			...
			{
				writeD(player.getAppearance().getSex() ? 0x01 : 0x00);
			...
			writeD(m.isOnline() ? m.getObjectId() : 0x00);
