...
public final class RequestExEnchantSkillChange extends GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		Skill skill = SkillsParser.getInstance().getInfo(_skillId, _skillLvl);
		if (skill == null)
		{
			return;
		}
		
		final int[] requestItems = EnchantSkillManager.getInstance().getEnchantItems(_skillId, 3, _skillLvl);
		...
		if (player.getSp() >= requiredSp)
		{
			...
			skill = SkillsParser.getInstance().getInfo(_skillId, _skillLvl);
			
			if (skill != null)
			{
				if (Config.LOG_SKILL_ENCHANTS)
				{
					Log.addLogEnchantSkill("ROUTE CHANGE", skill, player, spb, 0, false);
				}
			...
			if (levelPenalty == 0)
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.SKILL_ENCHANT_CHANGE_SUCCESSFUL_S1_LEVEL_WILL_REMAIN);
				sm.addSkillName(_skillId);
				player.sendPacket(sm);
			}
			else
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.SKILL_ENCHANT_CHANGE_SUCCESSFUL_S1_LEVEL_WAS_DECREASED_BY_S2);
				sm.addSkillName(_skillId);
