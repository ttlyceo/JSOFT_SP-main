...
public final class RequestExEnchantSkill extends GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		final int costMultiplier = EnchantSkillGroupsParser.NORMAL_ENCHANT_COST_MULTIPLIER;
		final int requiredSp = esd.getSpCost() * costMultiplier;
		if (player.getSp() >= requiredSp)
		{
			final boolean usesBook = (_skillLvl % 100) == 1;
			final int[] requestItems = EnchantSkillManager.getInstance().getEnchantItems(_skillId, 0, _skillLvl);
			final ItemInstance spb = player.getInventory().getItemByItemId(requestItems[0]);
			
			if (Config.ES_SP_BOOK_NEEDED && usesBook && (spb == null) || (spb != null && spb.getCount() < requestItems[1]))
			{
			...
			if (Rnd.get(100) <= rate)
			{
				...
				{
					Log.addLogEnchantSkill("SUCCESS", skill, player, spb, rate, true);
				}
			...
			{
				player.addSkill(SkillsParser.getInstance().getInfo(_skillId, s.getBaseLevel()), true);
				...
				if (Config.LOG_SKILL_ENCHANTS)
				{
					Log.addLogEnchantSkill("FAIL", skill, player, spb, rate, true);
				}
			}
			player.sendUserInfo();
