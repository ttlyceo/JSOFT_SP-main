...
public final class RequestActionUse extends GameClientPacket
{
	...
	{
	        NpcStringId.USING_A_SPECIAL_SKILL_HERE_COULD_TRIGGER_A_BLOODBATH, NpcStringId.HEY_WHAT_DO_YOU_EXPECT_OF_ME, NpcStringId.UGGGGGH_PUSH_ITS_NOT_COMING_OUT, NpcStringId.AH_I_MISSED_THE_MARK
	};
	...
	protected void runImpl()
	{
		...
		if ((activeChar.isFakeDeathNow() && (_actionId != 0)) || activeChar.isDead() || activeChar.isOutOfControl())
		{
			sendActionFailed();
			return;
		}
		...
		Effect ef = null;
		if (((ef = activeChar.getFirstEffect(EffectType.ACTION_BLOCK)) != null) && !ef.checkCondition(_actionId))
		{
		...
		if (activeChar.isTransformed())
		{
			...
			{
				sendActionFailed();
				Util.handleIllegalPlayerAction(activeChar, "" + activeChar.getName(null) + " used action which he does not have! Id = " + _actionId);
				if (Config.DEBUG)
				{
		...
		switch (_actionId)
		{
			case 0 :
				...
				{
					final NextAction nextAction = new NextAction(CtrlEvent.EVT_ARRIVED, CtrlIntention.MOVING, new NextActionCallback()
					{
						@Override
						public void doWork()
						{
			...
			case 7 :
				...
				for (final Creature cha : World.getInstance().getAroundCharacters(activeChar, 400, 200))
				{
					if (cha != null && !cha.isAlikeDead())
					{
						if ((nearestTarget == null || activeChar.getDistance3D(cha) < activeChar.getDistance3D(nearestTarget)) && cha.isAutoAttackable(activeChar, false))
						{
							nearestTarget = cha;
						}
					}
				}
				if (nearestTarget != null && activeChar.getTarget() != nearestTarget)
				{
					activeChar.setTarget(nearestTarget);
					return;
				}
				break;
			case 10 :
			...
			case 16 :
				if (validateSummon(summon, true))
				{
					if (summon.canAttack(target, _ctrlPressed))
					{
						summon.doAttack();
					}
				}
			...
			case 53 :
				if (validateSummon(summon, false))
				{
					if ((target != null) && (summon != target) && !summon.isMovementDisabled())
					{
						...
						summon.getAI().setIntention(CtrlIntention.MOVING, target.getLocation(), 0);
					}
				}
				break;
			case 54 :
			...
			default :
				if (Config.DEBUG)
				{
					_log.warn(activeChar.getName(null) + ": unhandled action type " + _actionId);
				}
				break;
		}
	}
	...
	private void useSkill(int skillId, GameObject target, boolean pet)
	{
		...
		if (skill != null)
		{
			...
			summon.useMagic(skill, _ctrlPressed, _shiftPressed, true);
		}
	...
	{
		final Player requester = getActiveChar();
		if (requester == null)
		{
			return;
		}

		final GameObject target = requester.getTarget();
		...
		if (requester.isActionsDisabled())
		{
			sendActionFailed();
			return;
		}
		...
		if (requester.isInFightEvent() && !requester.getFightEvent().isFriend(requester, (Creature) target))
		{
