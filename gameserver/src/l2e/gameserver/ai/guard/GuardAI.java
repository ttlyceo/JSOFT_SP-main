...
public class GuardAI extends CharacterAI implements Runnable
{
	...
	protected boolean checkAggression(Creature target)
	{
		...
		if (player == null || (player.getClan() != null && (player.getClan().getCastleId() == ((Npc) _actor).getCastle().getId())))
		{
			return false;
		}
	...
	private final void factionNotifyAndSupport()
	{
		...
		for (final var cha : World.getInstance().getAroundCharacters(_actor, 1000, 200))
		{
			...
			{
				if (_healSkills.length != 0 && (cha instanceof Player) && (((Npc) _actor).getCastle().getSiege().checkIsDefender(((Player) cha).getClan())))
				{
	...
	private void attackPrepare()
	{
		var dist2 = 0.;
		var range = 0;
		final var sGuard = (DefenderInstance) _actor;
		var attackTarget = getAttackTarget();
		
		try
		{
			_actor.setTarget(attackTarget);
			final var combinedCollision = (int) (_actor.getColRadius() + attackTarget.getColRadius());
			final var dist = Math.sqrt(_actor.getPlanDistanceSq(attackTarget.getX(), attackTarget.getY()));
			dist2 = (int) dist - _actor.getColRadius();
			...
			else
			{
				...
				else
				{
					...
					if ((dz * dz) < (170 * 170))
					{
						if (attackTarget != null)
						{
							moveTo(attackTarget.getLocation());
						}
					}
				}
			}
