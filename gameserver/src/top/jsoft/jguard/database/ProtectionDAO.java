package top.jsoft.jguard.database;
...
public class ProtectionDAO {
    ...
    private static final String INSERT_BAN_SQL_QUERY = "INSERT INTO hwid_bans (hwid, comment, ban_type, name) values (?,?,?,?)";
    ...
    public HashMap<Integer, HWIDInfo> loadHWIDListBanned() {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                hwidBan.setName(rset.getString("name"));
    ...
    public void storeHWIDBanned(final String hwid, final String comment, final BanType banType) {
        Connection con = null;
        PreparedStatement statement = null;
        try {
            con = DatabaseFactory.getInstance().getConnection();
    ...
    public void storeHWIDBanned(final GameClient client, final String comment, final BanType banType) {
        ...
        try {
            ...
            statement.setString(2, comment);
            statement.setString(3, banType.toString());
            statement.setString(4, banType == BanType.ACCOUNT_BAN ? client.getLogin() : client.getActiveChar() != null ? client.getActiveChar().getName(null) : "N/A");
            statement.execute();
        } catch (final SQLException e) {
            LOGGER.error("Store error storeHWIDBanned(Class, String, Class)", e);
        } finally {
